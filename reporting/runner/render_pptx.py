"""render_pptx.py — executive PowerPoint deck for the Budget Utilization report.

build_deck(sections, ctx) -> .pptx bytes. Consumes the SAME MULTI sections the
Briefing Book (budget_util_book.html.j2) renders — overview / by_sector /
pressure / ap_lines / grn_lines / open_po / open_pr — and produces a
leadership-facing 16:9 deck with NATIVE PowerPoint tables and charts (fully
editable in PowerPoint), so every figure matches the PDF/Excel outputs.

Slides: cover · executive KPIs · utilization by sector (chart+table) · budget
composition (doughnut) · budget lines under pressure · actuals & top suppliers ·
open obligations & commitments · observations & insights · methodology.

Design language mirrors the Briefing Book: deep teal + gold on white, prepared
