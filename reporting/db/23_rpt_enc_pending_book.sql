...
BEGIN
  ...
  l_secr := q'!SELECT NVL(coa.sector_name,'(Unclassified)') AS sector, COUNT(DISTINCT x.source||'~'||x.doc_number) AS docs, COUNT(*) AS lines, SUM(x.line_aed) AS amount_aed, SUM(CASE WHEN x.pending_days > 30 THEN x.line_aed ELSE 0 END) AS over30_aed FROM prod.dct_pr_po_pending_v x LEFT JOIN prod.dct_gl_coa_snap coa ON coa.cc_string = x.cc_string WHERE x.in_extract = 'Y' AND x.budget_year = [COLON]year!' || l_scope || q'! GROUP BY NVL(coa.sector_name,'(Unclassified)') ORDER BY 4 DESC!';
  -- part 3: pending PR register (line grain, oldest first)
  l_prr := q'!SELECT x.doc_number, x.doc_line, x.descr AS description, x.preparer_buyer, x.submitted_date, x.pending_days, x.pending_with, x.project_number, x.project_name, x.task_number, x.expenditure_type, x.budget_date, x.currency, x.line_aed, x.funds_status FROM prod.dct_pr_po_pending_v x WHERE x.in_extract = 'Y' AND x.source = 'PR' AND x.budget_year = [COLON]year!' || l_scope || q'! ORDER BY x.pending_days DESC, x.line_aed DESC!';
  -- part 4: pending PO register (line grain, oldest first; open = GRN-netted)
  l_por := q'!SELECT x.doc_number, x.doc_line, x.descr AS supplier_name, x.preparer_buyer, x.submitted_date, x.pending_days, x.pending_with, x.project_number, x.project_name, x.task_number, x.expenditure_type, x.budget_date, x.line_aed, x.enc_open_aed AS open_aed, x.funds_status FROM prod.dct_pr_po_pending_v x WHERE x.in_extract = 'Y' AND x.source = 'PO' AND x.budget_year = [COLON]year!' || l_scope || q'! ORDER BY x.pending_days DESC, x.line_aed DESC!';
  -- part 2b: the 20 longest-waiting documents
  l_old := q'!SELECT source, doc_number, description, preparer_buyer, submitted_date, pending_days, pending_with, doc_amt FROM !' || l_docg || q'! ORDER BY pending_days DESC, doc_amt DESC FETCH FIRST 20 ROWS ONLY!';
  ...
  l_um := q'!SELECT x.source, x.business_unit, x.doc_number, x.preparer_buyer, x.submitted_date, x.pending_days, x.pending_with FROM prod.dct_pr_po_pending_v x WHERE x.in_extract = 'N' ORDER BY x.pending_days DESC!';
