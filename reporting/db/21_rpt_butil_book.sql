...
DECLARE
  ...
  l_pls    VARCHAR2(10000);
  l_pld    VARCHAR2(10000);
  l_pcv    VARCHAR2(10000);
  l_src    CLOB;
  l_body   CLOB;
BEGIN
  -- shared predicates -- VERBATIM mirror of the GL /gl/butil page handler
  -- (GL/db/07): sector exact, projecttype pipe-any-of, task/etype/search contains-match.
  ...
  l_ov := q'!SELECT COUNT(DISTINCT sector) AS sectors, COUNT(DISTINCT project_number) AS projects, COUNT(DISTINCT project_number||'~'||NVL(task_number,'~')) AS tasks, COUNT(*) AS budget_lines, SUM(budget_annual+cadj_ovra) AS budget_annual, SUM(budget+cadj_ovr) AS budget, SUM(actual_ap+cadj_amt) AS actual_ap, SUM(actual_grn) AS actual_grn, SUM(actual_ap+cadj_amt)+SUM(actual_grn) AS actual_total, SUM(obligation_po) AS obligation_po, SUM(commitment_pr) AS commitment_pr, SUM(fund_available+cadj_ovr-cadj_amt) AS fund_available, ROUND(100*(SUM(actual_ap+cadj_amt)+SUM(actual_grn)+SUM(commitment_pr)+SUM(obligation_po))/NULLIF(SUM(budget+cadj_ovr),0),1) AS utilization_pct, COUNT(CASE WHEN fund_available+cadj_ovr-cadj_amt < -0.005 THEN 1 END) AS over_budget_lines, SUM(pf.plan_appr_annual) AS plan_approved_annual, SUM(pf.plan_appr_ytd) AS plan_approved_ytd, SUM(pf.plan_rev_annual) AS plan_revised_annual, SUM(pf.plan_rev_ytd) AS plan_revised_ytd FROM (SELECT bv0.*, NVL(c.cost_adj_aed,0) AS cadj_amt, NVL(c.budget_ovr_aed,0) AS cadj_ovr, NVL(c.budget_ovr_annual,0) AS cadj_ovra FROM prod.dct_budget_utilization_v bv0 LEFT JOIN prod.dct_pa_cost_adj_butil_v c ON c.budget_year = bv0.budget_year AND c.project_number = bv0.project_number AND c.task_number = bv0.task_number AND c.expenditure_type = bv0.expenditure_type) bv LEFT JOIN (SELECT budget_year AS py, project_number AS pp, task_number AS pt, expenditure_type AS pe, plan_appr_annual, plan_appr_ytd, plan_rev_annual, plan_rev_ytd FROM prod.dct_project_cf_butil_v) pf ON pf.py = bv.budget_year AND pf.pp = bv.project_number AND pf.pt = bv.task_number AND pf.pe = bv.expenditure_type!' || l_bscope;
  -- part 1b: sector rollup (table + charts)
  l_sec := q'!SELECT NVL(sector,'(Unclassified)') AS sector, COUNT(DISTINCT project_number) AS projects, SUM(budget_annual+cadj_ovra) AS budget_annual, SUM(budget+cadj_ovr) AS budget, SUM(actual_ap+cadj_amt) AS actual_ap, SUM(actual_grn) AS actual_grn, SUM(obligation_po) AS obligation_po, SUM(commitment_pr) AS commitment_pr, SUM(fund_available+cadj_ovr-cadj_amt) AS fund_available, ROUND(100*(SUM(actual_ap+cadj_amt)+SUM(actual_grn)+SUM(commitment_pr)+SUM(obligation_po))/NULLIF(SUM(budget+cadj_ovr),0),1) AS utilization_pct, SUM(pf.plan_appr_annual) AS annual_plan, SUM(pf.plan_appr_ytd) AS ytd_plan FROM (SELECT bv0.*, NVL(c.cost_adj_aed,0) AS cadj_amt, NVL(c.budget_ovr_aed,0) AS cadj_ovr, NVL(c.budget_ovr_annual,0) AS cadj_ovra FROM prod.dct_budget_utilization_v bv0 LEFT JOIN prod.dct_pa_cost_adj_butil_v c ON c.budget_year = bv0.budget_year AND c.project_number = bv0.project_number AND c.task_number = bv0.task_number AND c.expenditure_type = bv0.expenditure_type) bv LEFT JOIN (SELECT budget_year AS py, project_number AS pp, task_number AS pt, expenditure_type AS pe, plan_appr_annual, plan_appr_ytd FROM prod.dct_project_cf_butil_v) pf ON pf.py = bv.budget_year AND pf.pp = bv.project_number AND pf.pt = bv.task_number AND pf.pe = bv.expenditure_type!' || l_bscope || q'! GROUP BY NVL(sector,'(Unclassified)') ORDER BY 3 DESC!';
  -- part 1c: budget lines under pressure (least remaining funds)
  l_prs := q'!SELECT sector, project_number, project_name, task_number, expenditure_type, budget + cadj_ovr AS budget, actual_ap + cadj_amt + actual_grn AS actual, obligation_po, commitment_pr, fund_available + cadj_ovr - cadj_amt AS fund_available, ROUND(100*(actual_ap+cadj_amt+actual_grn+commitment_pr+obligation_po)/NULLIF(budget+cadj_ovr,0),1) AS utilization_pct FROM (SELECT bv0.*, NVL(c.cost_adj_aed,0) AS cadj_amt, NVL(c.budget_ovr_aed,0) AS cadj_ovr, NVL(c.budget_ovr_annual,0) AS cadj_ovra FROM prod.dct_budget_utilization_v bv0 LEFT JOIN prod.dct_pa_cost_adj_butil_v c ON c.budget_year = bv0.budget_year AND c.project_number = bv0.project_number AND c.task_number = bv0.task_number AND c.expenditure_type = bv0.expenditure_type) bv!' || l_bscope || q'! AND budget + cadj_ovr > 0 ORDER BY fund_available ASC NULLS LAST FETCH FIRST 15 ROWS ONLY!';
  ...
  l_ap := q'!SELECT x.project_number, x.project_name, x.task_number, x.expenditure_type, CASE WHEN ABS(NVL(x.matched_var_aed,0)) > 0.005 THEN '('||x.variance_types||') '||x.invoice_number ELSE x.invoice_number END AS invoice_number, x.invoice_date, x.supplier_name, x.invoice_currency AS currency, x.ap_actual_aed AS matched_aed, x.payment_status FROM prod.dct_unpaid_invoices_v x WHERE x.budget_year = [COLON]year AND ABS(NVL(x.ap_actual_aed,0)) > 0.005!' || l_scope || q'! ORDER BY x.project_number, x.task_number, x.expenditure_type, x.invoice_date!';
  ...
  -- Part 6 (v1.88.0): Expenditure Plan Performance -- plan vs actual by sector
  l_pls := q'!SELECT NVL(sector,'(Unclassified)') AS sector, SUM(actual_ap+cadj_amt)+SUM(actual_grn) AS actual_total, SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) AS ytd_plan, SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END) AS annual_plan, SUM(budget_annual+cadj_ovra) AS annual_budget, ROUND(100*(SUM(actual_ap+cadj_amt)+SUM(actual_grn))/NULLIF(SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END),0),1) AS exec_pct, ROUND(100*SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END)/NULLIF(SUM(budget_annual+cadj_ovra),0),1) AS coverage_pct, CASE WHEN NVL(SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END),0) <= 0.005 THEN 'Not planned' WHEN 100*(SUM(actual_ap+cadj_amt)+SUM(actual_grn))/SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) < MAX(th.thx1) THEN 'Below plan' WHEN 100*(SUM(actual_ap+cadj_amt)+SUM(actual_grn))/SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) > MAX(th.thx2) THEN 'Ahead of plan' ELSE 'Within plan' END AS plan_status FROM (SELECT bv0.*, NVL(c.cost_adj_aed,0) AS cadj_amt, NVL(c.budget_ovr_aed,0) AS cadj_ovr, NVL(c.budget_ovr_annual,0) AS cadj_ovra FROM prod.dct_budget_utilization_v bv0 LEFT JOIN prod.dct_pa_cost_adj_butil_v c ON c.budget_year = bv0.budget_year AND c.project_number = bv0.project_number AND c.task_number = bv0.task_number AND c.expenditure_type = bv0.expenditure_type) bv LEFT JOIN (SELECT budget_year AS py, project_number AS pp, task_number AS pt, expenditure_type AS pe, plan_appr_annual, plan_appr_ytd, plan_rev_annual, plan_rev_ytd FROM prod.dct_project_cf_butil_v) pf ON pf.py = bv.budget_year AND pf.pp = bv.project_number AND pf.pt = bv.task_number AND pf.pe = bv.expenditure_type CROSS JOIN (SELECT NVL(MAX(CASE s.setting_key WHEN 'PLAN_EXEC_TOL_LOW' THEN TO_NUMBER(s.setting_value) END),90) AS thx1, NVL(MAX(CASE s.setting_key WHEN 'PLAN_EXEC_TOL_HIGH' THEN TO_NUMBER(s.setting_value) END),110) AS thx2, NVL(MAX(CASE s.setting_key WHEN 'PLAN_COV_LOW' THEN TO_NUMBER(s.setting_value) END),95) AS thc1, NVL(MAX(CASE s.setting_key WHEN 'PLAN_COV_HIGH' THEN TO_NUMBER(s.setting_value) END),105) AS thc2 FROM prod.dct_module_settings s JOIN prod.dct_modules md ON md.module_id = s.module_id AND md.module_code = 'GL' WHERE s.setting_key LIKE 'PLAN%') th!' || l_bscope || q'! GROUP BY NVL(sector,'(Unclassified)') ORDER BY 4 DESC NULLS LAST!';
  -- top absolute deviations from the YTD plan (planned lines only)
  l_pld := q'!SELECT sector, project_number, project_name, task_number, expenditure_type, ROUND(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END,2) AS ytd_plan, actual_ap+actual_grn AS actual_total, ROUND(actual_ap+cadj_amt+actual_grn-(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END),2) AS variance, ROUND(100*(actual_ap+cadj_amt+actual_grn)/NULLIF(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END,0),1) AS exec_pct, CASE WHEN 100*(actual_ap+cadj_amt+actual_grn)/(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) < th.thx1 THEN 'Below plan' WHEN 100*(actual_ap+cadj_amt+actual_grn)/(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) > th.thx2 THEN 'Ahead of plan' ELSE 'Within plan' END AS plan_status FROM (SELECT bv0.*, NVL(c.cost_adj_aed,0) AS cadj_amt, NVL(c.budget_ovr_aed,0) AS cadj_ovr, NVL(c.budget_ovr_annual,0) AS cadj_ovra FROM prod.dct_budget_utilization_v bv0 LEFT JOIN prod.dct_pa_cost_adj_butil_v c ON c.budget_year = bv0.budget_year AND c.project_number = bv0.project_number AND c.task_number = bv0.task_number AND c.expenditure_type = bv0.expenditure_type) bv LEFT JOIN (SELECT budget_year AS py, project_number AS pp, task_number AS pt, expenditure_type AS pe, plan_appr_annual, plan_appr_ytd, plan_rev_annual, plan_rev_ytd FROM prod.dct_project_cf_butil_v) pf ON pf.py = bv.budget_year AND pf.pp = bv.project_number AND pf.pt = bv.task_number AND pf.pe = bv.expenditure_type CROSS JOIN (SELECT NVL(MAX(CASE s.setting_key WHEN 'PLAN_EXEC_TOL_LOW' THEN TO_NUMBER(s.setting_value) END),90) AS thx1, NVL(MAX(CASE s.setting_key WHEN 'PLAN_EXEC_TOL_HIGH' THEN TO_NUMBER(s.setting_value) END),110) AS thx2, NVL(MAX(CASE s.setting_key WHEN 'PLAN_COV_LOW' THEN TO_NUMBER(s.setting_value) END),95) AS thc1, NVL(MAX(CASE s.setting_key WHEN 'PLAN_COV_HIGH' THEN TO_NUMBER(s.setting_value) END),105) AS thc2 FROM prod.dct_module_settings s JOIN prod.dct_modules md ON md.module_id = s.module_id AND md.module_code = 'GL' WHERE s.setting_key LIKE 'PLAN%') th!' || l_bscope || q'! AND NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END,0) > 0.005 ORDER BY ABS(actual_ap+cadj_amt+actual_grn-(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END)) DESC FETCH FIRST 15 ROWS ONLY!';
  -- coverage + verdict-count summary over the whole scope (kv)
  l_pcv := q'!SELECT SUM(actual_ap+cadj_amt)+SUM(actual_grn) AS actual_total, SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) AS ytd_plan, SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END) AS annual_plan, SUM(budget_annual+cadj_ovra) AS annual_budget, ROUND(100*(SUM(actual_ap+cadj_amt)+SUM(actual_grn))/NULLIF(SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END),0),1) AS exec_pct, ROUND(100*SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END)/NULLIF(SUM(budget_annual+cadj_ovra),0),1) AS coverage_pct, SUM(CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END,0) > 0.005 AND 100*(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END)/NULLIF(budget_annual+cadj_ovra,0) < th.thc1 THEN budget_annual+cadj_ovra-(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END) ELSE 0 END) AS plan_gap, COUNT(CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END,0) > 0.005 AND 100*(actual_ap+cadj_amt+actual_grn)/(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) BETWEEN th.thx1 AND th.thx2 THEN 1 END) AS lines_within, COUNT(CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END,0) > 0.005 AND 100*(actual_ap+cadj_amt+actual_grn)/(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) < th.thx1 THEN 1 END) AS lines_below, SUM(CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END,0) > 0.005 AND 100*(actual_ap+cadj_amt+actual_grn)/(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) < th.thx1 THEN (CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END)-(actual_ap+cadj_amt+actual_grn) ELSE 0 END) AS below_amount, COUNT(CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END,0) > 0.005 AND 100*(actual_ap+cadj_amt+actual_grn)/(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) > th.thx2 THEN 1 END) AS lines_ahead, SUM(CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END,0) > 0.005 AND 100*(actual_ap+cadj_amt+actual_grn)/(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) > th.thx2 THEN (actual_ap+cadj_amt+actual_grn)-(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) ELSE 0 END) AS ahead_amount, COUNT(CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END,0) <= 0.005 THEN 1 END) AS lines_no_plan, COUNT(CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END,0) > 0.005 AND 100*(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END)/NULLIF(budget_annual+cadj_ovra,0) BETWEEN th.thc1 AND th.thc2 THEN 1 END) AS lines_fully, COUNT(CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END,0) > 0.005 AND 100*(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END)/NULLIF(budget_annual+cadj_ovra,0) < th.thc1 THEN 1 END) AS lines_under, COUNT(CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END,0) > 0.005 AND (NVL(budget_annual+cadj_ovra,0) = 0 OR 100*(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END)/(budget_annual+cadj_ovra) > th.thc2) THEN 1 END) AS lines_over, COUNT(CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END,0) <= 0.005 THEN 1 END) AS lines_unplanned FROM (SELECT bv0.*, NVL(c.cost_adj_aed,0) AS cadj_amt, NVL(c.budget_ovr_aed,0) AS cadj_ovr, NVL(c.budget_ovr_annual,0) AS cadj_ovra FROM prod.dct_budget_utilization_v bv0 LEFT JOIN prod.dct_pa_cost_adj_butil_v c ON c.budget_year = bv0.budget_year AND c.project_number = bv0.project_number AND c.task_number = bv0.task_number AND c.expenditure_type = bv0.expenditure_type) bv LEFT JOIN (SELECT budget_year AS py, project_number AS pp, task_number AS pt, expenditure_type AS pe, plan_appr_annual, plan_appr_ytd, plan_rev_annual, plan_rev_ytd FROM prod.dct_project_cf_butil_v) pf ON pf.py = bv.budget_year AND pf.pp = bv.project_number AND pf.pt = bv.task_number AND pf.pe = bv.expenditure_type CROSS JOIN (SELECT NVL(MAX(CASE s.setting_key WHEN 'PLAN_EXEC_TOL_LOW' THEN TO_NUMBER(s.setting_value) END),90) AS thx1, NVL(MAX(CASE s.setting_key WHEN 'PLAN_EXEC_TOL_HIGH' THEN TO_NUMBER(s.setting_value) END),110) AS thx2, NVL(MAX(CASE s.setting_key WHEN 'PLAN_COV_LOW' THEN TO_NUMBER(s.setting_value) END),95) AS thc1, NVL(MAX(CASE s.setting_key WHEN 'PLAN_COV_HIGH' THEN TO_NUMBER(s.setting_value) END),105) AS thc2 FROM prod.dct_module_settings s JOIN prod.dct_modules md ON md.module_id = s.module_id AND md.module_code = 'GL' WHERE s.setting_key LIKE 'PLAN%') th!' || l_bscope;
  ...
  l_src := '{"orientation":"landscape","required":["year"],'
        ...
        || '{"key":"plan_sector","title":"Plan Performance - by Sector","layout":"table","sql":"' || l_pls || '"}' || ','
        || '{"key":"plan_dev","title":"Plan Performance - Top Deviations","layout":"table","sql":"' || l_pld || '"}' || ','
        || '{"key":"plan_cov","title":"Plan Performance - Coverage","layout":"kv","sql":"' || l_pcv || '"}'
        || ']}';
