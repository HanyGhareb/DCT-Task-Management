...
BEGIN
  ...
  -- part 4: open commitments (reserved PR lines)
  l_pr := q'!SELECT x.project_number, x.project_name, x.task_number, x.expenditure_type, x.pr_number, x.description, x.budget_date, x.currency_code AS currency, x.distribution_amount, x.amount_aed FROM prod.dct_reserved_pr_lines_v x WHERE x.budget_year = [COLON]year!' || l_scope || q'! ORDER BY x.amount_aed DESC!';
