...
BEGIN
  ...
  -- BUDGET_COMBINATION = the line's natural key project.task.etype (col A,
  -- unique per row -- the analysts' lookup key). ACCOUNT_NUMBER = the code
  -- part of the view's GL_ACCOUNT (COA row txns, else the etype numeric
  -- prefix). The view ships gl_account/appropriation/program as
  -- 'CODE - Description' strings -- split on the FIRST ' - ' into code +
  -- name columns (desc may be absent).
  l_bu := q'!SELECT project_number || '.' || task_number || '.' || expenditure_type AS budget_combination, sector, chapter, CASE WHEN INSTR(appropriation,' - ') > 0 THEN SUBSTR(appropriation,1,INSTR(appropriation,' - ')-1) ELSE appropriation END AS appropriation_code, CASE WHEN INSTR(appropriation,' - ') > 0 THEN SUBSTR(appropriation,INSTR(appropriation,' - ')+3) END AS appropriation_name, CASE WHEN INSTR(program,' - ') > 0 THEN SUBSTR(program,1,INSTR(program,' - ')-1) ELSE program END AS dct_program_code, CASE WHEN INSTR(program,' - ') > 0 THEN SUBSTR(program,INSTR(program,' - ')+3) END AS dct_program_name, cost_centre, department, project_number, project_name, task_number, CASE WHEN INSTR(gl_account,' - ') > 0 THEN SUBSTR(gl_account,1,INSTR(gl_account,' - ')-1) ELSE gl_account END AS account_number, expenditure_type, budget_annual AS annual_budget, budget AS ytd_budget, actual_ap, actual_grn, actual_ap + actual_grn AS actual_total, commitment_pr, obligation_po, commitment_pr + obligation_po AS open_encumbrance, fund_available, ROUND(100*(actual_ap+actual_grn+commitment_pr+obligation_po)/NULLIF(budget,0),1) AS utilization_pct FROM prod.dct_budget_utilization_v!' || l_bscope || q'! ORDER BY sector, chapter, cost_centre, project_number, task_number, expenditure_type!';
