...
BEGIN
  ...
  l_bu := q'!SELECT sector, chapter, project_type, cost_centre, department, project_number, project_name, task_number, expenditure_type, budget, actual_ap, actual_grn, actual_ap + actual_grn AS actual_total, commitment_pr, obligation_po, commitment_pr + obligation_po AS open_encumbrance, fund_available, ROUND(100*(actual_ap+actual_grn+commitment_pr+obligation_po)/NULLIF(budget,0),1) AS utilization_pct FROM prod.dct_budget_utilization_v!' || l_bscope || q'! ORDER BY budget DESC NULLS LAST!';
