...
--            8. Fund Movement             (Additional Fund budget transfers
--               from the PBT extract; 2026-08-30 renamed from "Additional
--               Fund Transfers", columns re-ordered, Amount green/red)
...
BEGIN
  ...
  -- FUND_MOVEMENT_COUNT/_AMOUNT (2026-08-31, user) = the page's Fund
  -- Movement pair on the exact butil line key: APPROVED Additional Fund
  -- transfer lines only (the sheet-8 rule), trx_year = budget year,
  -- BUTIL_END transaction-date cut, zero-amount lines excluded -- values
  -- match the GL/db/34 line-level aggregates cell for cell. Amount alias
  -- __pn = green/red by sign. The renamed fm inline view (fp/ft/fe) keeps
  -- l_bscope's unqualified columns unambiguous (the pf-join ORA-00918 rule).
  -- TASK_NAME (2026-08-31, user) = ATD_TASKS.task_name PROJECT-SCOPED via
  -- ATD_PROJECTS (task numbers repeat across projects -- the tsk_org rule);
  -- the butil view has no task-name column and DCT_TASKS names are empty.
  -- Renamed tnm inline view (tp/tt), same ambiguity rule as fm/pf.
  l_bu := q'!SELECT budget_combination, sector, chapter, CASE WHEN INSTR(appropriation,' - ') > 0 THEN SUBSTR(appropriation,1,INSTR(appropriation,' - ')-1) ELSE appropriation END AS appropriation_code, CASE WHEN INSTR(appropriation,' - ') > 0 THEN SUBSTR(appropriation,INSTR(appropriation,' - ')+3) END AS appropriation_name, CASE WHEN INSTR(program,' - ') > 0 THEN SUBSTR(program,1,INSTR(program,' - ')-1) ELSE program END AS dct_program_code, CASE WHEN INSTR(program,' - ') > 0 THEN SUBSTR(program,INSTR(program,' - ')+3) END AS dct_program_name, cost_centre, department, task_organization AS organization, project_number, project_name, task_number, tnm.task_name AS task_name, CASE WHEN INSTR(gl_account,' - ') > 0 THEN SUBSTR(gl_account,1,INSTR(gl_account,' - ')-1) ELSE gl_account END AS account_number, (SELECT LISTAGG(m.ebs_value,' / ') WITHIN GROUP (ORDER BY m.ebs_value) FROM prod.dct_gl_ebs_map m WHERE m.segment_type = 'ACCOUNT' AND m.is_active = 'Y' AND m.fusion_value = CASE WHEN INSTR(gl_account,' - ') > 0 THEN SUBSTR(gl_account,1,INSTR(gl_account,' - ')-1) ELSE gl_account END) AS ebs_account, expenditure_type, budget_annual + bv.cadj_ovra AS annual_budget, budget + bv.cadj_ovr AS ytd_budget, ROUND(100*(actual_ap+bv.cadj_amt+actual_grn)/NULLIF(budget_annual+bv.cadj_ovra,0),1) AS budget_utilization_pct, CASE WHEN ABS(NVL(budget_annual+bv.cadj_ovra,0)) > 0.005 THEN ROUND(actual_ap+bv.cadj_amt+actual_grn-(budget_annual+bv.cadj_ovra),2) END AS budget_variance, CASE WHEN ABS(NVL(budget_annual+bv.cadj_ovra,0)) <= 0.005 THEN CASE WHEN actual_ap+bv.cadj_amt+actual_grn > 0.005 THEN TO_CHAR(NCHR(9660))||' Over budget' ELSE 'Not budgeted' END WHEN 100*(actual_ap+bv.cadj_amt+actual_grn)/(budget_annual+bv.cadj_ovra) > th.tbo THEN TO_CHAR(NCHR(9660))||' Over budget' WHEN 100*(actual_ap+bv.cadj_amt+actual_grn)/(budget_annual+bv.cadj_ovra) >= th.tbn THEN TO_CHAR(NCHR(9650))||' Near limit' ELSE TO_CHAR(NCHR(9679))||' On track' END AS budget_status, pf.plan_appr_annual AS plan_annual, pf.plan_appr_ytd AS plan_ytd, pf.plan_rev_annual AS revised_plan_annual, pf.plan_rev_ytd AS revised_plan_ytd, ROUND(100*(actual_ap+bv.cadj_amt+actual_grn)/NULLIF(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END,0),1) AS plan_utilization_pct, CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END,0) > 0.005 THEN ROUND(actual_ap+bv.cadj_amt+actual_grn-(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END),2) END AS plan_variance, CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END,0) <= 0.005 THEN 'Not planned' WHEN 100*(actual_ap+bv.cadj_amt+actual_grn)/(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) < th.thx1 THEN TO_CHAR(NCHR(9660))||' Below plan' WHEN 100*(actual_ap+bv.cadj_amt+actual_grn)/(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_ytd ELSE pf.plan_appr_ytd END) > th.thx2 THEN TO_CHAR(NCHR(9650))||' Ahead of plan' ELSE TO_CHAR(NCHR(9679))||' Within plan' END AS plan_status, ROUND(100*NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END,0)/NULLIF(budget_annual+bv.cadj_ovra,0),1) AS plan_coverage_pct, CASE WHEN NVL(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END,0) <= 0.005 THEN 'Not planned' WHEN NVL(budget_annual+bv.cadj_ovra,0) = 0 OR 100*(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END)/(budget_annual+bv.cadj_ovra) > th.thc2 THEN 'Over-planned' WHEN 100*(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN pf.plan_rev_annual ELSE pf.plan_appr_annual END)/(budget_annual+bv.cadj_ovra) < th.thc1 THEN 'Under-planned' ELSE 'Fully planned' END AS plan_coverage_status, actual_ap + bv.cadj_amt AS actual_ap, actual_grn, actual_ap + bv.cadj_amt + actual_grn AS actual_total, commitment_pr, obligation_po, commitment_pr + obligation_po AS open_encumbrance, fund_available + bv.cadj_ovr - bv.cadj_amt AS fund_available, ROUND(100*(actual_ap+bv.cadj_amt+actual_grn+commitment_pr+obligation_po)/NULLIF(budget+bv.cadj_ovr,0),1) AS utilization_pct, fm.fm_cnt AS fund_movement_count, fm.fm_amt AS fund_movement_amount__pn, CASE WHEN [COLON]cmtmode IN ('PERIOD','ALL') THEN (SELECT LISTAGG('['||c.accounting_period||'] '||c.created_by||' - '||c.comment_text, CHR(10) ON OVERFLOW TRUNCATE) WITHIN GROUP (ORDER BY c.created_at DESC) FROM prod.dct_gl_butil_comment c WHERE c.status = 'ACTIVE' AND c.entity_level = 'BUTIL_LINE' AND c.budget_year = bv.budget_year AND c.project_number = bv.project_number AND c.task_number = bv.task_number AND c.expenditure_type = bv.expenditure_type AND ([COLON]cmtmode = 'ALL' OR SYS_CONTEXT('GL_CTX','BUTIL_END') IS NULL OR c.accounting_period = TO_CHAR(TO_DATE(SYS_CONTEXT('GL_CTX','BUTIL_END'),'YYYY-MM-DD'),'MM-YYYY'))) END AS comments FROM (SELECT bv0.*, NVL(c.cost_adj_aed,0) AS cadj_amt, NVL(c.budget_ovr_aed,0) AS cadj_ovr, NVL(c.budget_ovr_annual,0) AS cadj_ovra FROM prod.dct_budget_utilization_v bv0 LEFT JOIN prod.dct_pa_cost_adj_butil_v c ON c.budget_year = bv0.budget_year AND c.project_number = bv0.project_number AND c.task_number = bv0.task_number AND c.expenditure_type = bv0.expenditure_type) bv LEFT JOIN (SELECT budget_year AS py, project_number AS pp, task_number AS pt, expenditure_type AS pe, plan_appr_annual, plan_appr_ytd, plan_rev_annual, plan_rev_ytd FROM prod.dct_project_cf_butil_v) pf ON pf.py = bv.budget_year AND pf.pp = bv.project_number AND pf.pt = bv.task_number AND pf.pe = bv.expenditure_type LEFT JOIN (SELECT l.project_num AS fp, l.task_num AS ft, l.expenditure_type AS fe, COUNT(*) AS fm_cnt, SUM(l.additional_amount) AS fm_amt FROM prod.pa_additional_fund_lines l JOIN prod.pa_budget_trx_headers h ON h.transaction_num = l.transaction_num AND h.transaction_type = 'Additional' JOIN (SELECT transaction_num, MAX(assignment_state) KEEP (DENSE_RANK LAST ORDER BY seq_no) AS st FROM prod.pa_budget_trx_approvals WHERE trx_type = 'Additional' GROUP BY transaction_num) fa ON fa.transaction_num = l.transaction_num AND fa.st = 'Approved' WHERE h.trx_year = TO_CHAR([COLON]year) AND (SYS_CONTEXT('GL_CTX','BUTIL_END') IS NULL OR h.transaction_date < TO_DATE(SYS_CONTEXT('GL_CTX','BUTIL_END'),'YYYY-MM-DD') + 1) AND NVL(l.additional_amount,0) <> 0 GROUP BY l.project_num, l.task_num, l.expenditure_type) fm ON fm.fp = bv.project_number AND fm.ft = bv.task_number AND fm.fe = bv.expenditure_type LEFT JOIN (SELECT p.project_number AS tp, t.task_number AS tt, MAX(t.task_name) AS task_name FROM prod.atd_tasks t JOIN prod.atd_projects p ON p.project_id = t.project_id GROUP BY p.project_number, t.task_number) tnm ON tnm.tp = bv.project_number AND tnm.tt = bv.task_number CROSS JOIN (SELECT NVL(MAX(CASE s.setting_key WHEN 'PLAN_EXEC_TOL_LOW' THEN TO_NUMBER(s.setting_value) END),90) AS thx1, NVL(MAX(CASE s.setting_key WHEN 'PLAN_EXEC_TOL_HIGH' THEN TO_NUMBER(s.setting_value) END),110) AS thx2, NVL(MAX(CASE s.setting_key WHEN 'PLAN_COV_LOW' THEN TO_NUMBER(s.setting_value) END),95) AS thc1, NVL(MAX(CASE s.setting_key WHEN 'PLAN_COV_HIGH' THEN TO_NUMBER(s.setting_value) END),105) AS thc2, NVL(MAX(CASE s.setting_key WHEN 'BUD_UTIL_NEAR_PCT' THEN TO_NUMBER(s.setting_value) END),90) AS tbn, NVL(MAX(CASE s.setting_key WHEN 'BUD_UTIL_OVER_PCT' THEN TO_NUMBER(s.setting_value) END),100) AS tbo FROM prod.dct_module_settings s JOIN prod.dct_modules md ON md.module_id = s.module_id AND md.module_code = 'GL' WHERE (s.setting_key LIKE 'PLAN%' OR s.setting_key LIKE 'BUD_UTIL%')) th!' || l_bscope || q'! ORDER BY sector, chapter, cost_centre, project_number, task_number, expenditure_type!';
  ...
  l_src := '{"required":["year"],'
        ...
        || '{"key":"budget_trx","title":"Fund Movement","layout":"table","sql":"' || l_bt || '"}'
        || ']}';
  ...
  l_body :=
    ...
    'open requisitions, the pending-approval PR/PO queue and the Fund Movement sheet (Additional Fund budget transfers) ({{ row_count }} lines in total).</p>'||
  ...
  VALUES
    ('BUDGET_UTIL_REGISTER',
     ...
     'Excel register for internal analysis: the Budget Utilization detail lists, each in its own worksheet -- utilization lines (budget + expenditure plan (annual and YTD, revised pair when uploaded) vs AP / GRN / open PR / open PO / fund available + fund-movement count/amount per project, task and expenditure type), direct AP invoices (including the approved costing-adjustment transactions, marked (**) with their Cost Adjustment Ref), GRN receipts (receipt-date year basis), open purchase orders (GRN-netted), open requisitions (reserved) the pending-approval PR/PO queue (funds-reserved) and, when cmtmode is PERIOD/ALL, the Budget Utilization comments (line comments as a sheet-1 column, all other levels in their own worksheet) and the Fund Movement sheet (Approved Additional Fund transfers; amount green/red). Parameters mirror the GL Budget Utilization page filters: year (required); period (YTD, MM-YYYY), sector, chapter, projecttype, costcenter, project, task, etype, search (all optional).',
