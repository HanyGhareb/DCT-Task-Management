...
BEGIN
  ...
  -- shared dimension join for the line sheets (2026-07-30): Sector /
  -- Cost centre / Department per butil key from the scope view (one row per
  -- key -- cannot fan out; same source as sheet 1's dimensions)
  l_dim := q'! LEFT JOIN (SELECT budget_year, project_number, NVL(task_number,'~') AS tk, NVL(expenditure_type,'~') AS et, MAX(sector) AS sector, MAX(cost_centre) AS cost_centre, MAX(department) AS department FROM prod.dct_butil_scope_v GROUP BY budget_year, project_number, NVL(task_number,'~'), NVL(expenditure_type,'~')) sc ON sc.budget_year = [COLON]year AND sc.project_number = x.project_number AND sc.tk = NVL(x.task_number,'~') AND sc.et = NVL(x.expenditure_type,'~')!';
