...
-- ----------------------------------------------------------------------------
-- V_IFW_USER_PERMISSIONS
-- Flattened permission codes per active user (via roles).
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW v_ifw_user_permissions AS
SELECT DISTINCT
    u.user_id,
    u.username,
    p.permission_code,
...
JOIN  ifw_users           u  ON ur.user_id      = u.user_id
JOIN  ifw_roles           r  ON ur.role_id      = r.role_id
JOIN  ifw_role_permissions rp ON rp.role_id     = r.role_id
JOIN  ifw_permissions     p  ON rp.permission_id = p.permission_id
LEFT JOIN ifw_modules     m  ON p.module_id     = m.module_id
WHERE ur.is_active  = 'Y'
  AND u.is_active   = 'Y'
  AND r.is_active   = 'Y'
  AND p.is_active   = 'Y'
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW v_ifw_module_access AS
SELECT DISTINCT
    u.user_id,
    u.username,
    mo.module_id,
...
JOIN  ifw_users         u  ON ur.user_id   = u.user_id
JOIN  ifw_roles         r  ON ur.role_id   = r.role_id
JOIN  ifw_module_roles  mr ON mr.role_id   = r.role_id
JOIN  ifw_modules       mo ON mr.module_id = mo.module_id
WHERE ur.is_active  = 'Y'
  AND u.is_active   = 'Y'
  AND r.is_active   = 'Y'
  AND mo.is_active  = 'Y'
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW v_ifw_pending_approvals AS
SELECT
    ai.instance_id,
    ai.source_module,
    ai.source_record_id,
    ...
    mo.module_name_en,
    mo.module_name_ar
FROM  ifw_approval_instances ai
JOIN  ifw_approval_templates at2 ON ai.template_id     = at2.template_id
JOIN  ifw_approval_steps     aps ON aps.template_id    = ai.template_id
                                 AND aps.step_seq       = ai.current_step_seq
JOIN  ifw_users              u_sub ON ai.submitted_by  = u_sub.user_id
LEFT JOIN ifw_roles          r  ON aps.required_role_id = r.role_id
LEFT JOIN ifw_modules        mo ON at2.module_id        = mo.module_id
WHERE ai.overall_status = 'PENDING';

-- ----------------------------------------------------------------------------
-- V_IFW_ACTIVE_DELEGATIONS
-- Delegations currently in effect (today falls in the date range).
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW v_ifw_active_delegations AS
SELECT
    d.delegation_id,
    d.delegator_id,
    ...
    d.end_date,
    d.reason
FROM  ifw_delegations  d
JOIN  ifw_users        u_from ON d.delegator_id = u_from.user_id
JOIN  ifw_users        u_to   ON d.delegate_id  = u_to.user_id
LEFT JOIN ifw_roles    r      ON d.role_id      = r.role_id
LEFT JOIN ifw_modules  m      ON d.module_id    = m.module_id
WHERE d.status     = 'ACTIVE'
  AND TRUNC(SYSDATE) BETWEEN TRUNC(d.start_date) AND TRUNC(d.end_date);

-- ----------------------------------------------------------------------------
-- V_IFW_USER_DIRECTORY
-- Full user profile view for display in the UI (no sensitive columns).
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW v_ifw_user_directory AS
SELECT
    u.user_id,
    u.username,
    u.email,
    ...
    o.org_type
FROM  ifw_users         u
LEFT JOIN ifw_organizations o ON u.org_id = o.org_id;
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW v_ifw_org_tree AS
SELECT
    o.org_id,
    o.org_code,
    o.org_name_en,
    ...
    (SELECT COUNT(*) FROM ifw_users uu WHERE uu.org_id = o.org_id AND uu.is_active = 'Y') AS member_count
FROM  ifw_organizations o
LEFT JOIN ifw_organizations p ON o.parent_org_id = p.org_id
LEFT JOIN ifw_users         u ON o.head_user_id  = u.user_id;
...
-- ----------------------------------------------------------------------------
-- dct_data_access_assignment  (was a table; now a view over IFW_USER_ROLES)
