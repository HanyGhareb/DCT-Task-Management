...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW PROD.v_dct_user_active_roles AS
SELECT
    u.user_id,
    u.username,
    u.display_name,
...
LEFT JOIN PROD.dct_modules    m  ON r.module_id = m.module_id
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW PROD.v_dct_user_permissions AS
SELECT DISTINCT
    u.user_id,
    u.username,
    p.permission_code,
...
JOIN  PROD.dct_users           u  ON ur.user_id      = u.user_id
JOIN  PROD.dct_roles           r  ON ur.role_id      = r.role_id
JOIN  PROD.dct_role_permissions rp ON rp.role_id     = r.role_id
JOIN  PROD.dct_permissions     p  ON rp.permission_id = p.permission_id
LEFT JOIN PROD.dct_modules     m  ON p.module_id     = m.module_id
WHERE ur.is_active  = 'Y'
  AND u.is_active   = 'Y'
  AND r.is_active   = 'Y'
  AND p.is_active   = 'Y'
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW PROD.v_dct_module_access AS
SELECT DISTINCT
    u.user_id,
    u.username,
    mo.module_id,
...
JOIN  PROD.dct_users         u  ON ur.user_id   = u.user_id
JOIN  PROD.dct_roles         r  ON ur.role_id   = r.role_id
JOIN  PROD.dct_module_roles  mr ON mr.role_id   = r.role_id
JOIN  PROD.dct_modules       mo ON mr.module_id = mo.module_id
WHERE ur.is_active  = 'Y'
  AND u.is_active   = 'Y'
  AND r.is_active   = 'Y'
  AND mo.is_active  = 'Y'
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW PROD.v_dct_pending_approvals AS
SELECT
    ai.instance_id,
    ai.source_module,
    ai.source_record_id,
...
JOIN  PROD.dct_approval_templates at2 ON ai.template_id     = at2.template_id
JOIN  PROD.dct_approval_steps     aps ON aps.template_id    = ai.template_id
                                 AND aps.step_seq       = ai.current_step_seq
JOIN  PROD.dct_users              u_sub ON ai.submitted_by  = u_sub.user_id
LEFT JOIN PROD.dct_roles          r  ON aps.required_role_id = r.role_id
LEFT JOIN PROD.dct_modules        mo ON at2.module_id        = mo.module_id
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW PROD.v_dct_active_delegations AS
SELECT
    d.delegation_id,
    d.delegator_id,
...
JOIN  PROD.dct_users        u_from ON d.delegator_id = u_from.user_id
JOIN  PROD.dct_users        u_to   ON d.delegate_id  = u_to.user_id
LEFT JOIN PROD.dct_roles    r      ON d.role_id      = r.role_id
LEFT JOIN PROD.dct_modules  m      ON d.module_id    = m.module_id
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW PROD.v_dct_user_directory AS
SELECT
    u.user_id,
    u.username,
    u.email,
...
LEFT JOIN PROD.dct_organizations o ON u.org_id = o.org_id;
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW PROD.v_dct_org_tree AS
SELECT
    o.org_id,
    o.org_code,
    o.org_name_en,
    ...
    (SELECT COUNT(*) FROM PROD.dct_users uu WHERE uu.org_id = o.org_id AND uu.is_active = 'Y') AS member_count
FROM  PROD.dct_organizations o
LEFT JOIN PROD.dct_organizations p ON o.parent_org_id = p.org_id
LEFT JOIN PROD.dct_users         u ON o.head_user_id  = u.user_id;
...
-- ----------------------------------------------------------------------------
-- PROD.dct_data_access_assignment  (was a table; now a view over DCT_USER_ROLES)
-- Existing apps query: entity_type_id='ROLE', status='A', user_id=:APP_USER
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW PROD.dct_data_access_assignment AS
SELECT
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE VIEW PROD.roles AS
SELECT
    r.role_id,
    r.role_code,
    r.role_name_en   AS role_name,
