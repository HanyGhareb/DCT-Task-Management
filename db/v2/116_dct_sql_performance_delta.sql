...
CREATE OR REPLACE PACKAGE BODY prod.dct_sql_perf_pkg AS
  ...
  BEGIN
    ...
    SELECT sql_id,plan_hash_value,
      ...
      FROM (
        SELECT q.*,
         ...
         WHERE q.executions-b.executions >= l_min
    ...
    SELECT COUNT(*),NVL(SUM(CASE WHEN avg_seconds>=l_alert THEN 1 ELSE 0 END),0),NVL(MAX(avg_seconds),0)
    ...
    IF l_status='WARNING' AND NVL(l_prev,'-')<>l_sig THEN
      FOR u IN (
        ...
        prod.dct_notify.send(u.user_id,'WARNING','Slow application SQL detected',
          l_slow||' application SQL statement(s) averaged at least '||l_alert||
          ' seconds during the latest 30-minute interval. Slowest average: '||l_top||' seconds.',
