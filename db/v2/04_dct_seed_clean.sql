...
-- Sprint  : 1 — Foundation
-- =============================================================================
-- Inserts: Modules, Roles, Permissions, Role-Permission maps,
--          System Settings, Lookup Categories & Values,
--          Default SYS_ADMIN user, Root Organisation
-- =============================================================================
-- IMPORTANT: Change the default admin password immediately after first login.
-- Default admin credentials: username=ADMIN / password=iFinance@2026
-- =============================================================================

-- =============================================================================
-- 1. ROOT ORGANISATION
-- =============================================================================
INSERT INTO PROD.dct_organizations (org_code, org_name_en, org_name_ar, org_type, level_no, full_path, created_by)
...
INTO PROD.dct_role_permissions (role_id, permission_id, granted_by)
...
INTO PROD.dct_role_permissions (role_id, permission_id, granted_by)
    SELECT r.role_id, p.permission_id, 'SYSTEM'
    FROM PROD.dct_roles r JOIN PROD.dct_permissions p
...
INTO PROD.dct_role_permissions (role_id, permission_id, granted_by)
    SELECT r.role_id, p.permission_id, 'SYSTEM'
    FROM PROD.dct_roles r JOIN PROD.dct_permissions p ON p.permission_code = 'TASKS.DIRECTOR_VIEW'
    WHERE r.role_code = 'TASK_DIRECTOR'
INTO PROD.dct_role_permissions (role_id, permission_id, granted_by)
    SELECT r.role_id, p.permission_id, 'SYSTEM'
    FROM PROD.dct_roles r JOIN PROD.dct_permissions p ON p.permission_code = 'TASKS.MANAGE'
    WHERE r.role_code = 'TASK_MANAGER'
INTO PROD.dct_role_permissions (role_id, permission_id, granted_by)
    SELECT r.role_id, p.permission_id, 'SYSTEM'
    FROM PROD.dct_roles r JOIN PROD.dct_permissions p ON p.permission_code = 'TASKS.VIEW'
...
INSERT ALL
-- USER_STATUS
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, is_default, created_by)
    SELECT c.category_id, 'ACTIVE',   'Active',   'نشط',    1, 'Y', 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'USER_STATUS'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'INACTIVE', 'Inactive', 'غير نشط', 2, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'USER_STATUS'
-- ORG_TYPE
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'DIVISION',   'Division',   'دائرة',   1, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'ORG_TYPE'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'SECTION',    'Section',    'قسم',     2, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'ORG_TYPE'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'UNIT',       'Unit',       'وحدة',    3, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'ORG_TYPE'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'DEPARTMENT', 'Department', 'إدارة',   4, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'ORG_TYPE'
-- TASK_STATUS
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'NOT_STARTED', 'Not Started', 'لم تبدأ',    1, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'TASK_STATUS'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'IN_PROGRESS', 'In Progress', 'قيد التنفيذ', 2, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'TASK_STATUS'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'COMPLETED',   'Completed',   'مكتملة',     3, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'TASK_STATUS'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'DELAYED',     'Delayed',     'متأخرة',     4, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'TASK_STATUS'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'BLOCKED',     'Blocked',     'معلقة',      5, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'TASK_STATUS'
-- TASK_PRIORITY
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'HIGH',   'High',   'عالية',   1, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'TASK_PRIORITY'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'MEDIUM', 'Medium', 'متوسطة',  2, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'TASK_PRIORITY'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'LOW',    'Low',    'منخفضة',  3, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'TASK_PRIORITY'
-- APPROVAL_ACTION
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'APPROVED',  'Approved',          'معتمد',            1, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'APPROVAL_ACTION'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'REJECTED',  'Rejected',          'مرفوض',            2, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'APPROVAL_ACTION'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'RETURNED',  'Returned for Review','مُعاد للمراجعة',  3, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'APPROVAL_ACTION'
INTO PROD.dct_lookup_values (category_id, value_code, value_name_en, value_name_ar, display_order, created_by)
    SELECT c.category_id, 'DELEGATED', 'Delegated',          'مُفوَّض',          4, 'SYSTEM' FROM PROD.dct_lookup_categories c WHERE c.category_code = 'APPROVAL_ACTION'
SELECT 1 FROM DUAL;

COMMIT;

-- =============================================================================
-- 9. DEFAULT SYS_ADMIN USER
-- Username: ADMIN
-- Password: iFinance@2026   ← CHANGE IMMEDIATELY AFTER FIRST LOGIN
-- =============================================================================
DECLARE
    l_org_id  NUMBER;
    l_user_id NUMBER;
...
BEGIN
    ...
    INSERT INTO PROD.dct_users (
        ...
        color_hex, language_pref, created_by
    ) VALUES (
        'ADMIN', 'admin@ifinance.local',
    ...
    INSERT INTO PROD.dct_user_roles (
        ...
        'Initial system administrator account', 'SYSTEM'
    );
    ...
    INSERT INTO PROD.dct_user_roles (
        ...
        l_user_id, l_role_id, SYSDATE, 'Y', 'SYSTEM',
        'Base platform access', 'SYSTEM'
    );

    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Default admin user created. user_id = ' || l_user_id);
    DBMS_OUTPUT.PUT_LINE('*** CHANGE THE ADMIN PASSWORD IMMEDIATELY ***');
END;
/

-- =============================================================================
-- 10. SEED ORGANISATIONS (Finance Division sections from CLAUDE.md)
-- =============================================================================
DECLARE
    l_div_id NUMBER;
BEGIN
    SELECT org_id INTO l_div_id FROM PROD.dct_organizations WHERE org_code = 'FINANCE_DIV';

    INSERT INTO PROD.dct_organizations (org_code, org_name_en, org_name_ar, org_type, parent_org_id, level_no, full_path, created_by)
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Finance Division sections seeded.');
END;
/
