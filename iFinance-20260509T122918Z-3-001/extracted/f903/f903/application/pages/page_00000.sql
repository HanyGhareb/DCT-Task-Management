prompt --application/pages/page_00000
begin
--   Manifest
--     PAGE: 00000
--   Manifest End
wwv_flow_api.component_begin (
 p_version_yyyy_mm_dd=>'2020.03.31'
,p_release=>'20.1.0.00.13'
,p_default_workspace_id=>1448684262833497
,p_default_application_id=>114
,p_default_id_offset=>9604171250607172
,p_default_owner=>'DEV'
);
wwv_flow_api.create_page(
 p_id=>0
,p_user_interface_id=>wwv_flow_api.id(65623444952255812)
,p_name=>'Global Page - Desktop'
,p_step_title=>'Global Page - Desktop'
,p_autocomplete_on_off=>'OFF'
,p_page_template_options=>'#DEFAULT#'
,p_protection_level=>'D'
,p_last_updated_by=>'TCA9051'
,p_last_upd_yyyymmddhh24miss=>'20200529112310'
);
wwv_flow_api.create_page_da_event(
 p_id=>wwv_flow_api.id(76390546409025213)
,p_name=>'UserProfile'
,p_event_sequence=>10
,p_bind_type=>'bind'
,p_bind_event_type=>'ready'
,p_display_when_type=>'USER_IS_NOT_PUBLIC_USER'
);
wwv_flow_api.create_page_da_action(
 p_id=>wwv_flow_api.id(76390615508025214)
,p_event_id=>wwv_flow_api.id(76390546409025213)
,p_event_result=>'TRUE'
,p_action_sequence=>10
,p_execute_on_page_init=>'N'
,p_action=>'PLUGIN_APEX.PROFILE.MENU'
,p_attribute_01=>wwv_flow_string.join(wwv_flow_t_varchar2(
'{',
'"buttonPosition": "right",',
'"iconPosition"  : "Left", ',
'"buttonType"    : "success"',
'}'))
,p_attribute_02=>'has-username'
,p_attribute_04=>wwv_flow_string.join(wwv_flow_t_varchar2(
'SELECT ',
'first_name || '' '' || last_name  USERNAME,',
'''http://s1defp1.dev.uae:7001/ords/dev/profile/view/'' || user_name IMAGE, ',
'--''f?p=&APP_ID.:41:&SESSION.::::'' EDIT_LINK, ',
'apex_util.prepare_url( ''f?p=''||:APP_ID||'':41:''||:APP_SESSION||''::NO::P41_USER_NAME:''|| user_name ) EDIT_LINK,',
'email EMAIL, ',
'mobile_sms PHONE_NO,',
'(select desc_e',
'        from dct_employees_lookups',
'        where lookup_name = ''Positions Codes''',
'        and code = to_char(dct_employees_list2.position_id)) DESIGNATION,',
'(select org.org_name_en ',
'    from dct_hr_organizations org',
'    where org.org_id =  organization_id) DEPARTMENT,',
'''Department Culture and Tourism AD'' COMPANY',
'FROM dct_employees_list2',
'where user_name = :APP_USER;'))
,p_attribute_05=>'Y'
);
wwv_flow_api.component_end;
end;
/
