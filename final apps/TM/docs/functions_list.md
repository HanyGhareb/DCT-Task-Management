...
| Team Vacations (**DRAFT** — sample data until `/tm/vacations` ships) | `views/vacations.html` + `viewModels/vacations.js` | `load`, **`setScale`** (Day/Week/Month/Year time scales), `setView` (Month layout: calendar ↔ team grid), `prev`/`next`/`goToday` (scale-aware step), `drillMonth` (Year cell → that Month), `openVac`, `openAdd`/`dayClick`/`gridCellClick`/`editVac`/`saveForm`/`removeVac` (local in draft). **Day** = agenda of who's out; **Week** = member×7-day grid; **Month** = APEX-style calendar with lane-laid leave bars or per-member team grid; **Year** = member×12-month leave-day heat matrix + team totals. Scale-aware KPI band (on-leave-today / away-in-period / leave-days / peak-overlap) + validated 5-type legend; team selector scoped by `listTeams`/`listMembers`. Member avatars render the **profile photo** (`/dct/users/:id/photo` media handler) with `photoErr` initials fallback (`selVacPhoto`/`selVacInitials` for the detail modal) |
