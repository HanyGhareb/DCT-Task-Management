...
-- =============================================================================
-- 12_cc_inbox_view.sql
-- CC: serve the approvals inbox from the SHARED worklist view.
-- Schema : ADMIN (ORDS metadata). Own SQLcl session.
-- Run    : AFTER db/v2/64 (views) and db/v2/66 (ADMIN synonyms).
--
-- Retires CC's private copy of the inbox query. Eligibility now has ONE
-- definition platform-wide (PROD.DCT_WF_INBOX_V). CC's own response keys are
...
PROMPT === CC: approvals inbox -> DCT_WF_INBOX_V ===

BEGIN
    ORDS.DEFINE_HANDLER(
        p_module_name => 'cc.rest',
...
END;
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('cc.rest approvals/pending now reads the shared inbox view.');
END;
/

PROMPT === 12_cc_inbox_view.sql complete ===
