...
BEGIN
  ...
  FOR m IN (
    SELECT TO_CHAR( dct_to_local(ADD_MONTHS(TRUNC(SYSDATE,'MM'), 1 - LEVEL)), 'Mon YYYY') AS month_label,
...
BEGIN
  ...
  FOR r IN (SELECT * FROM dct_cc_card_v WHERE cc_id = [COLON]id) LOOP
    ...
    APEX_JSON.write('updatedAt',        TO_CHAR( dct_to_local(FROM_TZ(CAST(r.updated_at AS TIMESTAMP), 'UTC') AT TIME ZONE 'Asia/Dubai'),'DD-Mon-YYYY HH:MI AM'));
    APEX_JSON.close_object;
  END LOOP;
EXCEPTION WHEN OTHERS THEN dct_rest.err(500, SQLERRM);
END;
...
BEGIN
  ...
  FOR r IN (
    ...
    APEX_JSON.write('changedAt',  TO_CHAR( dct_to_local(r.changed_at), 'YYYY-MM-DD'));
...
BEGIN
  ...
  FOR r IN (
    ...
    APEX_JSON.write('submittedAt',       TO_CHAR( dct_to_local(r.submitted_at), 'YYYY-MM-DD'));
...
!');
    ...
    -- Raw-binary upload: the file's bytes ARE the request body (no base64, no
    -- ~32 KB cap). Per the AR note, :body may be dereferenced only ONCE. File
    -- name / mime ride in the query string. Size guarded by MAX_UPLOAD_MB.
...
BEGIN
  ...
  BEGIN
    ...
    WHERE  m.module_code = 'CREDIT_CARDS' AND ms.setting_key = 'MAX_UPLOAD_MB';
...
BEGIN
  ...
  FOR r IN (SELECT * FROM dct_cc_replenishment_v WHERE replenishment_id = [COLON]id) LOOP
    ...
    FOR ln IN (
      ...
      APEX_JSON.write('expenseDate',     TO_CHAR( dct_to_local(ln.expense_date), 'YYYY-MM-DD'));
