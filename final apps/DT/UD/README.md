# UD — User Stories (Duty Travel, App 204)
...
DT/UD/
  ...
  UD-DT-01_<First-Story-Title>/
    UD-DT-01_<First-Story-Title>.md     ← the story pack (spec + Dev/Test/Deploy trackers)
    assets/                                 ← optional: mockups, diagrams, screenshots
  UD-DT-02_<Next-Story>/
    ...
```

## Naming

- **ID:** `UD-DT-<NN>` — app code + zero-padded 2-digit sequence (`UD-DT-01`, `UD-DT-02`, …).
  Sequence is **global-sequential per app** and never reused.
- **Folder:** `UD-DT-<NN>_<Short-Title-Kebab>`.
...
*No stories yet. The first will be `UD-DT-01_<Short-Title>` — create its folder here when work starts.*
