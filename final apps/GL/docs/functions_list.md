...
| GET | `/butil` | Budget Utilization report, `?year(req)=&projecttype=&sector=&costcenter=&project=&task=&etype=&search=&limit=&offset=` → totals + items over `DCT_BUDGET_UTILIZATION_V` (db/v2/37); `costcenter`/`project`/`task`/`etype` are contains-match LOV filters (project matches number OR name) |
| GET | `/butil/lines` | Budget Utilization figure drill-down, `?year(req)=&metric(req)=` (ap\|grn\|pr\|po) + EITHER `&project=&task=&etype=` (row drill) OR `&projecttype=&sector=&search=&costcenter=&fproject=&ftask=&fetype=` (KPI-card aggregate; the LOV filters are f-prefixed to avoid the row-drill key params) → `{metric, aggregate, total, count, columns[], rows[]}`; one `kys` key-set CTE drives both modes; filters/amounts mirror `DCT_BUDGET_UTILIZATION_V` so totals reconcile to the on-screen figure; rows capped 1000 (window `total`/`count` are full). PO/PR header+line joins de-duped to avoid fan-out. **Every drawer shows document # + line** (ap = Invoice #/Line via `line_number`, grn = GRN #/Line via `receipt_line_number` **+ PO #/PO Line/Supplier** via de-duped `po_headers`/`po_lines` joins, pr = PR #/Line via `pr_lines.pr_line`, po = PO #/Line) and **zero-amount lines are suppressed** (ap/grn/pr amount ≠ 0, po open > 0) (`07_gl_budget_util_ords.sql`, ADDITIVE — re-run after any 05 re-run) |
