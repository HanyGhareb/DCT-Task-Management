...
- business-question answer cards: `tot(k)` reads the `totals` summary (budget, **commitment (PR)**, **obligation (PO)**, **open commitment (PR)**, **open obligation (PO)**, encumbrance, GL actual, funds available, GRN actual, **AP direct**, **SLA actual**) for the filtered set.
- result table: budget · **commitment (+PR count)** · **obligation (+PO count)** · **open commitment** · **open obligation** · encumbrance · GL actual · funds · GRN · AP direct · **SLA actual** columns; full segment codes/descs per row reuse the combination hover popover (`comboHover`); the four PO-derived headers (+ SLA) carry a **visible ⓘ hint marker** (`.hint-i`); row-hover highlight. *Open* = the unliquidated subset of obligation — PO lines whose budget is still encumbered (FUNDS_STATUS Reserved/Partially Liquidated); open commitment is its PR-backed subset. **AP Direct** = AP distribution lines with **no PO** (`po_number IS NULL`). **SLA Actual** (Subledger Actuals) = GRN actual + AP direct (non-drillable subtotal). Commitment/Obligation cells show the amount + a count sub-line (`prCount` "N PRs" / `poCount` "N POs"; PR count interim from po_distributions until the PR_* tables land).
...
- Actuals reporting (db/v2/32–35): base pass-through views (`AP_*`/`PO_*`/`GRN_ALL_V2`/`GL_BALANCES`), `DCT_ACTUAL_V`, `DCT_BUDGET_ACTUAL_V`, `DCT_BUDGET_ACTUAL_PERIOD_V` (per-combination × period YTD; incl. `appropriation_code`/`_desc`, `commitment_ytd` [PR-backed PO lines] / `obligation_ytd` [all PO lines], `open_commitment_ytd` / `open_obligation_ytd` [the FUNDS_STATUS Reserved/Partially-Liquidated subset], `po_count`/`pr_count` [distinct PO headers / PR numbers per combination], read live from `po_distributions`; **`ap_direct_actual_ytd` = AP distribution lines with `po_number IS NULL`** — a true direct AP, classified by `cc_id`); indexed snapshot `DCT_GL_COA_SNAP` + `prod.dct_actuals_refresh` (hourly job `DCT_ACTUALS_REFRESH_JOB`).
