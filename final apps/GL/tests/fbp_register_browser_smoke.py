#!/usr/bin/env python3
"""GL browser smoke — FBP - Projects Budget Utilization menu entry (v1.120.0).

Verifies the Budget Utilization page's Generate Report dropdown carries the
new "FBP - Projects Budget Utilization (XLSX)" entry (EN + AR), and that
clicking it enqueues a run (busy state + "The Binder" popup with the rgGenFbp
label). The full render/download/layout path is covered by
fbp_register_api_smoke.py.

Run: python fbp_register_browser_smoke.py                    (webtier default)
     GL_BASE=http://localhost:8213 python fbp_register_browser_smoke.py
"""
import json
import os
import re
...
with sync_playwright() as p:
    ...
    check('dropdown has 5 entries', items.count() == 5, items.count())
    txt = page.locator('#pg-butil .gen-menu').inner_text()
    check('FBP entry (EN)', 'FBP - Projects Budget Utilization (XLSX)' in txt)
    fbp = page.locator('#pg-butil .gen-menu .gen-item',
                       has_text='FBP - Projects Budget Utilization')
    check('entry sits after Excel Register',
          'FBP - Projects Budget Utilization' in items.nth(3).inner_text())
    check('fixed-layout subtitle (EN)', 'Task Name' in fbp.inner_text())
    hint = fbp.get_attribute('title') or ''
    check('hint = fixed layout + Task Name',
          'FIXED' in hint and 'Task Name' in hint and 'Manage Columns' in hint)

    # click = enqueue: busy state + Binder popup with the rgGenFbp label
    # (wait for the filters LOV to land first — runBuFbp guards on buYear())
    page.wait_for_function("() => ko.dataFor(document.body).buYear()", timeout=60000)
    fbp.evaluate('el => el.click()')
    page.wait_for_function("() => ko.dataFor(document.body).buFbpBusy()", timeout=20000)
    check('buFbpBusy set', True)
    ...
    check('Binder popup label',
          'FBP - Projects Budget Utilization' in page.evaluate("%s.rgLabel()" % VM))
    ...
    check('AR entry label', 'FBP - استغلال ميزانية المشاريع (XLSX)' in txt_ar)
