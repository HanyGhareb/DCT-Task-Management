-- =============================================================================
-- General Ledger (App 210) -- FBP Projects Budget Utilization bridge (ADDITIVE)
-- File    : 52_gl_fbp_register_ords.sql
-- Adds to : gl.rest (does NOT delete/redefine the module)
-- Run     : sql -name prod_mcp @52_gl_fbp_register_ords.sql  (fresh session)
-- IMPORTANT: 05_gl_ords.sql DELETE_MODULEs gl.rest -- whenever 05 is re-run,
--            re-run 07..51 + THIS script right after it.
-- Purpose : runs FBP_BUTIL_REGISTER (reporting/db/45 -- the FBP distribution
--           copy of BUDGET_UTIL_REGISTER with the FIXED 21-column sheet-1
--           layout, Task Name in place of Task Number) straight from the GL
--           Budget Utilization page's Generate Report menu. Same filter set
--           as /gl/butil/xlsx; deliberately NO sheetcols forwarding -- the
--           layout is baked into the definition and the page's Manage-Columns
--           view must never affect it.
-- Endpoints:
--   POST /gl/butil/fbpxlsx          {year, period?, sector?, chapter?,
--                                    projecttype?, costcenter?, project?,
--                                    task?, etype?, search?, bu?, ovr?,
--                                    cmtmode?} -> {runId}
--   GET  /gl/butil/fbpxlsx/:id      -> {runId, status, rowCount, error, hasFile}
--   GET  /gl/butil/fbpxlsx/:id/file -> authed XLSX download of a finished run
-- =============================================================================
...
CREATE OR REPLACE PROCEDURE setup_gl_fbpreg_ords_tmp AS
...
BEGIN
    ...
    def_template('butil/fbpxlsx');
    def_handler('butil/fbpxlsx', 'POST', q'!
DECLARE
...
BEGIN
  ...
  -- Deliberately NO sheetcols: the FBP sheet-1 layout is FIXED in the
  -- definition (reporting/db/45) and Manage Columns must never change it.
  APEX_JSON.write('cmtmode', NVL(UPPER(APEX_JSON.get_varchar2(p_path=>'cmtmode')),'NONE'));
  APEX_JSON.close_object;
  l_params := APEX_JSON.get_clob_output;
  APEX_JSON.free_output;
  l_run := dct_rpt_pkg.enqueue(p_report_code  => 'FBP_BUTIL_REGISTER',
                               p_params       => l_params,
                               p_trigger      => 'ONDEMAND',
...
!');
    ...
    def_template('butil/fbpxlsx/[COLON]id');
    def_handler('butil/fbpxlsx/[COLON]id', 'GET', q'!
DECLARE
  l_user VARCHAR2(100) := dct_rest.validate_session;
  l_xls  NUMBER;
BEGIN
  ...
  FOR c IN (SELECT run_id, status, row_count, error_msg, started_at, finished_at
              FROM dct_rpt_run
             WHERE run_id = [COLON]id AND report_code = 'FBP_BUTIL_REGISTER') LOOP
...
!');
    ...
    def_template('butil/fbpxlsx/[COLON]id/file');
    def_handler('butil/fbpxlsx/[COLON]id/file', 'GET', q'!
DECLARE
  l_user VARCHAR2(100) := dct_rest.validate_session;
...
BEGIN
    ...
    SELECT o.file_blob, o.file_name, o.mime_type INTO l_blob, l_name, l_mime FROM (
      SELECT o.file_blob, o.file_name, o.mime_type
       ...
       WHERE o.run_id = [COLON]id AND o.format = 'XLSX'
         AND r.report_code = 'FBP_BUTIL_REGISTER'
  ...
  HTP.p('Content-Disposition[COLON] attachment; filename="'||NVL(l_name,'fbp_projects_budget_utilization.xlsx')||'"');
...
BEGIN
    setup_gl_fbpreg_ords_tmp;
    COMMIT;
END;
/
DROP PROCEDURE setup_gl_fbpreg_ords_tmp;
...
WHERE m.name = 'gl.rest'
  AND t.uri_template LIKE 'butil/fbpxlsx%'
ORDER BY t.uri_template, h.method;

PROMPT gl.rest FBP register bridge published (/gl/butil/fbpxlsx + /:id + /:id/file).
