-- =============================================================================
-- Budget Utilization -- PLAN INSIGHTS (ADDITIVE, DEFINE_HANDLER only)
-- File    : 32_butil_plan_insights.sql   App 210 / GL        2026-08-27
-- Adds to : gl.rest -- redefines ONLY the GET butil handler; THIS FILE IS NOW
--           THE LIVE OWNER of GET /gl/butil (supersedes 31, which superseded
--           28/21). Body = 31's exact deployed body (verified byte-identical
--           before this edit) + the plan-insight layer.
-- Run     : sql -name prod_mcp @32_butil_plan_insights.sql  (fresh session, ADMIN)
-- Needs   : db/v2/126 (DCT_PROJECT_CF_BUTIL_V) deployed. GL post-05 re-run
--           list is now 07..32.
--
-- WHAT IT ADDS (user-selected mockups D1 + K1 + K2, 2026-08-27):
-- 1. Per-row Plan-vs-Actual + Plan-vs-Budget verdicts computed SERVER-SIDE so
--    page, CSV and drills can never disagree:
--      planEffYtd / planEffAnnual  -- EFFECTIVE plan = REVISED when a revised
--                                     plan exists (rev annual <> 0), else APPROVED
--      planExecPct / planVariance / planExecState  WITHIN | BELOW | AHEAD | NOPLAN
--        (Actual = the row's displayed Total Actual: AP incl. cost-adj when
--         costadj=Y, + GRN, + procash when procash=Y, vs plan YTD)
--      planCovPct / planCovState  FULL | UNDER | OVER | NONE
--        (effective plan annual vs the row's displayed adjusted annual budget)
-- 2. Full-filtered-set aggregates in totals: planEffYtd/planEffAnnual,
--    planWithin/planBelow(+Amt)/planAhead(+Amt)/planNoPlan,
--    planCovFull/planCovUnder/planCovGap/planCovOver/planCovNone.
-- 3. NEW PARAMETER planstate=WITHIN|BELOW|AHEAD|NOPLAN -- restricts BOTH the
--    aggregate totals AND the items to lines with that verdict (the K1 tile
--    rows / K2 insight-strip "View lines" drills), same convention as negfund.
-- 4. planThresholds echo {execLow,execHigh,covLow,covHigh} -- read from GL
--    module settings PLAN_EXEC_TOL_LOW/HIGH + PLAN_COV_LOW/HIGH (seeded below,
--    defaults 90/110/95/105), so the client colors rows EXACTLY as the server
--    counted them.
-- SIZE WARNING: handler literal is now ~31.1K of the 32,767 PL/SQL literal cap
-- -- the NEXT addition to this handler MUST switch the define to concatenated
-- TO_CLOB pieces (see reference: ORDS handler 32k split) or it will silently
-- keep the OLD source.
-- LESSON carried from 31: extend the CURRENT deployed body, never an older
-- copy; after any future in-place edit of 21, re-run THIS file (not 28/31).
-- =============================================================================
...
CREATE OR REPLACE PROCEDURE setup_gl_butil_plan_ins AS
...
BEGIN
  ...
  FOR s IN (SELECT setting_key, setting_value FROM prod.dct_module_settings
             WHERE module_id = (SELECT module_id FROM prod.dct_modules WHERE module_code = 'GL')
               AND setting_key IN ('PLAN_EXEC_TOL_LOW','PLAN_EXEC_TOL_HIGH','PLAN_COV_LOW','PLAN_COV_HIGH')) LOOP
    CASE s.setting_key
...
END;!', '[COLON]', CHR(58)));
    COMMIT;
END setup_gl_butil_plan_ins;
/

BEGIN setup_gl_butil_plan_ins; END;
/
DROP PROCEDURE setup_gl_butil_plan_ins;
...
BEGIN
    ...
    up('PLAN_EXEC_TOL_LOW',  '90',  'Plan execution - lower tolerance (%)',
       'Actual below this % of the YTD expenditure plan = Below plan (hands down)');
    up('PLAN_EXEC_TOL_HIGH', '110', 'Plan execution - upper tolerance (%)',
       'Actual above this % of the YTD expenditure plan = Ahead of plan');
    up('PLAN_COV_LOW',  '95',  'Plan coverage - lower bound (%)',
       'Annual plan below this % of the annual budget = Under-planned');
    up('PLAN_COV_HIGH', '105', 'Plan coverage - upper bound (%)',
       'Annual plan above this % of the annual budget = Over-planned');
    COMMIT;
END;
/
...
SELECT LENGTH(h.source) AS handler_chars,
       CASE WHEN INSTR(h.source, 'l_plst') > 0 THEN 'PLANSTATE WIRED' ELSE 'MISSING' END AS state1,
       CASE WHEN INSTR(h.source, 'planCovGap') > 0 THEN 'INSIGHTS WIRED' ELSE 'MISSING' END AS state2,
       CASE WHEN INSTR(h.source, 'l_negf') > 0 THEN 'NEGFUND KEPT' ELSE 'MISSING' END AS state3,
       CASE WHEN INSTR(h.source, 'dct_project_cf_butil_v') > 0 THEN 'PLAN KEPT' ELSE 'MISSING' END AS state4
...
SELECT setting_key, setting_value FROM prod.dct_module_settings
 WHERE setting_key LIKE 'PLAN_%' ORDER BY setting_key;
