-- =============================================================================
-- Budget Utilization -- ACTUAL vs BUDGET verdict columns (DEFINE_HANDLER only)
-- File    : 35_butil_vs_budget.sql   App 210 / GL              2026-08-31
-- Adds to : gl.rest -- redefines ONLY the GET butil handler; THIS FILE IS NOW
--           THE LIVE OWNER of GET /gl/butil (supersedes 32 -> 31 -> 28 -> 21).
--           Body = 32's exact deployed body (verified before this edit) + the
--           vs-Budget layer. GL post-05 re-run list is now 07..35.
-- Run     : sql -name prod_mcp @35_butil_vs_budget.sql  (fresh session, ADMIN)
--
-- WHAT IT ADDS (user request 2026-08-31, mirrors the Plan Insights columns):
--   Per row: budUtilPct  = 100 * displayed Total Actual / ADJUSTED ANNUAL
--                          budget (user rule: Annual always, never YTD)
--            budVariance = Total Actual - Annual Budget (signed)
--            budState    = OK (< near) | NEAR (near..over) | OVER (> over)
--                          | NOBUDGET  -- spend on a zero-budget line = OVER
--   Thresholds: GL module settings BUD_UTIL_NEAR_PCT (90) / BUD_UTIL_OVER_PCT
--   (100), seeded below, echoed per response as budgetThresholds {near, over}
--   so the client colors rows EXACTLY as the server classified them.
--
-- THE DEFINE IS NOW ASSEMBLED FROM CLOB PIECES (the 32-file size warning):
-- each q'!...!' literal stays well under the 32,767 PL/SQL literal cap, so
-- future additions extend a piece or add one -- never re-grow a single
-- literal past the cap (it would silently keep the OLD source).
-- LESSON carried from 31/32: extend the CURRENT deployed body, never an older
-- copy; after any future in-place edit of 21, re-run THIS file.
-- =============================================================================
...
CREATE OR REPLACE PROCEDURE setup_gl_butil_vsbud AS
...
BEGIN
  ...
  -- planstate: WITHIN / BELOW / AHEAD / NOPLAN -- restricts totals AND items
  -- to lines with that Plan-vs-Actual verdict (K1 tile rows / K2 strip drills)
  l_plst   VARCHAR2(10)  := UPPER([COLON]planstate);
  -- plan-insight thresholds (GL module settings, Admin-editable; defaults 90/110/95/105)
  l_thx1 NUMBER := 90; l_thx2 NUMBER := 110; l_thc1 NUMBER := 95; l_thc2 NUMBER := 105;
  -- vs Budget verdict thresholds (Actual vs the ADJUSTED ANNUAL budget --
  -- user rule 2026-08-31: Annual always, never the YTD slice). Settings
  -- BUD_UTIL_NEAR_PCT / BUD_UTIL_OVER_PCT (defaults 90 / 100).
  ...
  t_pfe NUMBER := 0; t_pfea NUMBER := 0;
  t_piw NUMBER := 0; t_pib NUMBER := 0; t_piba NUMBER := 0;
  t_pia NUMBER := 0; t_piaa NUMBER := 0; t_pin NUMBER := 0;
  t_pcf NUMBER := 0; t_pcu NUMBER := 0; t_pcug NUMBER := 0; t_pco NUMBER := 0; t_pcn NUMBER := 0;
BEGIN
  ...
  IF l_plst NOT IN ('WITHIN','BELOW','AHEAD','NOPLAN') THEN l_plst := NULL; END IF;
  ...
  SELECT COUNT(*), NVL(SUM(budget),0), NVL(SUM(budget_annual),0), NVL(SUM(actual_ap),0), NVL(SUM(actual_grn),0),
         ...
         NVL(SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END),0),
         NVL(SUM(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END),0),
         COUNT(CASE WHEN CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END>0.005 AND 100*(CASE WHEN l_cadj='Y' THEN v.actual_ap+NVL(ca.cost_adj_aed,0) ELSE v.actual_ap END + v.actual_grn + CASE WHEN l_pcash='Y' THEN NVL(pc.procash_aed,0) ELSE 0 END)/NULLIF(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END,0) BETWEEN l_thx1 AND l_thx2 THEN 1 END),
         COUNT(CASE WHEN CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END>0.005 AND 100*(CASE WHEN l_cadj='Y' THEN v.actual_ap+NVL(ca.cost_adj_aed,0) ELSE v.actual_ap END + v.actual_grn + CASE WHEN l_pcash='Y' THEN NVL(pc.procash_aed,0) ELSE 0 END)/NULLIF(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END,0) < l_thx1 THEN 1 END),
         NVL(SUM(CASE WHEN CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END>0.005 AND 100*(CASE WHEN l_cadj='Y' THEN v.actual_ap+NVL(ca.cost_adj_aed,0) ELSE v.actual_ap END + v.actual_grn + CASE WHEN l_pcash='Y' THEN NVL(pc.procash_aed,0) ELSE 0 END)/NULLIF(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END,0) < l_thx1 THEN (CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END)-(CASE WHEN l_cadj='Y' THEN v.actual_ap+NVL(ca.cost_adj_aed,0) ELSE v.actual_ap END + v.actual_grn + CASE WHEN l_pcash='Y' THEN NVL(pc.procash_aed,0) ELSE 0 END) END),0),
         COUNT(CASE WHEN CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END>0.005 AND 100*(CASE WHEN l_cadj='Y' THEN v.actual_ap+NVL(ca.cost_adj_aed,0) ELSE v.actual_ap END + v.actual_grn + CASE WHEN l_pcash='Y' THEN NVL(pc.procash_aed,0) ELSE 0 END)/NULLIF(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END,0) > l_thx2 THEN 1 END),
         NVL(SUM(CASE WHEN CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END>0.005 AND 100*(CASE WHEN l_cadj='Y' THEN v.actual_ap+NVL(ca.cost_adj_aed,0) ELSE v.actual_ap END + v.actual_grn + CASE WHEN l_pcash='Y' THEN NVL(pc.procash_aed,0) ELSE 0 END)/NULLIF(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END,0) > l_thx2 THEN (CASE WHEN l_cadj='Y' THEN v.actual_ap+NVL(ca.cost_adj_aed,0) ELSE v.actual_ap END + v.actual_grn + CASE WHEN l_pcash='Y' THEN NVL(pc.procash_aed,0) ELSE 0 END)-(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END) END),0),
         COUNT(CASE WHEN CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END<=0.005 THEN 1 END),
         COUNT(CASE WHEN CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END>0.005 AND 100*(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END)/NULLIF(CASE WHEN l_cadj='Y' THEN v.budget_annual+NVL(ca.budget_ovr_annual,0) ELSE v.budget_annual END,0) BETWEEN l_thc1 AND l_thc2 THEN 1 END),
         COUNT(CASE WHEN CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END>0.005 AND 100*(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END)/NULLIF(CASE WHEN l_cadj='Y' THEN v.budget_annual+NVL(ca.budget_ovr_annual,0) ELSE v.budget_annual END,0) < l_thc1 THEN 1 END),
         NVL(SUM(CASE WHEN CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END>0.005 AND 100*(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END)/NULLIF(CASE WHEN l_cadj='Y' THEN v.budget_annual+NVL(ca.budget_ovr_annual,0) ELSE v.budget_annual END,0) < l_thc1 THEN (CASE WHEN l_cadj='Y' THEN v.budget_annual+NVL(ca.budget_ovr_annual,0) ELSE v.budget_annual END)-(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END) END),0),
         COUNT(CASE WHEN CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END>0.005 AND (100*(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END)/NULLIF(CASE WHEN l_cadj='Y' THEN v.budget_annual+NVL(ca.budget_ovr_annual,0) ELSE v.budget_annual END,0) > l_thc2 OR 100*(CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END)/NULLIF(CASE WHEN l_cadj='Y' THEN v.budget_annual+NVL(ca.budget_ovr_annual,0) ELSE v.budget_annual END,0) IS NULL) THEN 1 END),
         COUNT(CASE WHEN CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END<=0.005 THEN 1 END)
    INTO l_total, t_bud, t_buda, t_ap, t_grn, t_pr, t_po, t_fund, l_misscc, t_misscc,
         ...
         t_pfe, t_pfea, t_piw, t_pib, t_piba, t_pia, t_piaa, t_pin,
         t_pcf, t_pcu, t_pcug, t_pco, t_pcn
  ...
  IF l_plst IS NOT NULL THEN APEX_JSON.write('planState', l_plst); END IF;
  ...
  APEX_JSON.write('planEffYtd', t_pfe);
  APEX_JSON.write('planEffAnnual', t_pfea);
  APEX_JSON.write('planWithin', t_piw);
  APEX_JSON.write('planBelow', t_pib);
  APEX_JSON.write('planBelowAmt', t_piba);
  APEX_JSON.write('planAhead', t_pia);
  APEX_JSON.write('planAheadAmt', t_piaa);
  APEX_JSON.write('planNoPlan', t_pin);
  APEX_JSON.write('planCovFull', t_pcf);
  APEX_JSON.write('planCovUnder', t_pcu);
  APEX_JSON.write('planCovGap', t_pcug);
  APEX_JSON.write('planCovOver', t_pco);
  APEX_JSON.write('planCovNone', t_pcn);
  ...
  APEX_JSON.close_object;
!';
    l_src := l_src || q'!  APEX_JSON.open_array('items');
  FOR r IN (
    SELECT v.*, NVL(pc.procash_aed,0) AS procash,
           ...
           NVL(pf.cf_rows,0) AS cf_rows, tnm.task_name AS task_nm,
           CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_ytd,0) ELSE NVL(pf.plan_appr_ytd,0) END AS plan_eff_ytd,
           CASE WHEN NVL(pf.plan_rev_annual,0)<>0 THEN NVL(pf.plan_rev_annual,0) ELSE NVL(pf.plan_appr_annual,0) END AS plan_eff_annual,
           (CASE WHEN l_cadj='Y' THEN v.actual_ap+NVL(ca.cost_adj_aed,0) ELSE v.actual_ap END + v.actual_grn + CASE WHEN l_pcash='Y' THEN NVL(pc.procash_aed,0) ELSE 0 END) AS act_eff,
           CASE WHEN l_cadj='Y' THEN v.budget_annual+NVL(ca.budget_ovr_annual,0) ELSE v.budget_annual END AS buda_eff
      FROM prod.dct_budget_utilization_v v
      LEFT JOIN prod.dct_ap_procash_butil_v pc
             ON pc.budget_year      = v.budget_year
            AND pc.project_number   = v.project_number
      ...
      LEFT JOIN prod.dct_project_cf_butil_v pf
            ...
            AND pf.task_number      = v.task_number
            AND pf.expenditure_type = v.expenditure_type
      LEFT JOIN (SELECT p.project_number AS tp, t.task_number AS tt,
    ...
    APEX_JSON.write('taskName', NVL(r.task_nm,''));
    ...
    -- vs Budget (v1.96.0): the row's displayed Total Actual vs its ADJUSTED
    -- ANNUAL budget. OK below l_tbn%, NEAR l_tbn..l_tbo%, OVER above l_tbo%;
    -- spend on a zero-budget line = OVER, nothing at all = NOBUDGET.
    IF ABS(r.buda_eff) > 0.005 THEN
      APEX_JSON.write('budUtilPct', ROUND(100*r.act_eff/r.buda_eff,1));
      APEX_JSON.write('budVariance', ROUND(r.act_eff - r.buda_eff,2));
      APEX_JSON.write('budState',
        CASE WHEN 100*r.act_eff/r.buda_eff > l_tbo THEN 'OVER'
             WHEN 100*r.act_eff/r.buda_eff >= l_tbn THEN 'NEAR' ELSE 'OK' END);
    ELSE
      APEX_JSON.write('budState', CASE WHEN r.act_eff > 0.005 THEN 'OVER' ELSE 'NOBUDGET' END);
    END IF;
    ...
    APEX_JSON.write('planEffYtd', r.plan_eff_ytd);
    APEX_JSON.write('planEffAnnual', r.plan_eff_annual);
    IF r.plan_eff_ytd > 0.005 THEN
      APEX_JSON.write('planExecPct', ROUND(100*r.act_eff/r.plan_eff_ytd,1));
      APEX_JSON.write('planVariance', ROUND(r.act_eff - r.plan_eff_ytd,2));
      APEX_JSON.write('planExecState',
        CASE WHEN 100*r.act_eff/r.plan_eff_ytd < l_thx1 THEN 'BELOW'
             WHEN 100*r.act_eff/r.plan_eff_ytd > l_thx2 THEN 'AHEAD' ELSE 'WITHIN' END);
    ELSE
      APEX_JSON.write('planExecState', 'NOPLAN');
    END IF;
    IF r.plan_eff_annual > 0.005 THEN
      IF ABS(r.buda_eff) > 0.005 THEN
        APEX_JSON.write('planCovPct', ROUND(100*r.plan_eff_annual/r.buda_eff,1));
        APEX_JSON.write('planCovState',
          CASE WHEN 100*r.plan_eff_annual/r.buda_eff < l_thc1 THEN 'UNDER'
               WHEN 100*r.plan_eff_annual/r.buda_eff > l_thc2 THEN 'OVER' ELSE 'FULL' END);
      ELSE
...
END;!';
    ORDS.DEFINE_HANDLER(
        ...
        p_source      => REPLACE(l_src, '[COLON]', CHR(58)));
    COMMIT;
END setup_gl_butil_vsbud;
/

BEGIN setup_gl_butil_vsbud; END;
/
DROP PROCEDURE setup_gl_butil_vsbud;

PROMPT === threshold settings seed (GL module, Admin-editable) ===
DECLARE
    l_gl NUMBER;
    PROCEDURE up (p_key VARCHAR2, p_val VARCHAR2, p_label VARCHAR2, p_desc VARCHAR2) IS
        v NUMBER;
    BEGIN
        SELECT COUNT(*) INTO v FROM prod.dct_module_settings
         WHERE module_id = l_gl AND setting_key = p_key;
        IF v = 0 THEN
            INSERT INTO prod.dct_module_settings
                (module_id, setting_key, setting_value, setting_label,
                 setting_description, value_type, default_value, updated_by, updated_at)
            VALUES (l_gl, p_key, p_val, p_label, p_desc, 'NUMBER', p_val, 'SEED', SYSTIMESTAMP);
        END IF;
    END;
BEGIN
    SELECT module_id INTO l_gl FROM prod.dct_modules WHERE module_code = 'GL';
    up('BUD_UTIL_NEAR_PCT', '90',
       'Budget utilization - near-limit threshold (%)',
       'Actual at or above this % of the annual budget = Near limit on the vs Budget column');
    up('BUD_UTIL_OVER_PCT', '100',
       'Budget utilization - over-budget threshold (%)',
       'Actual above this % of the annual budget = Over budget on the vs Budget column');
    COMMIT;
END;
/
...
SELECT LENGTH(h.source) AS handler_chars,
       CASE WHEN INSTR(h.source, 'budUtilPct') > 0 THEN 'VSBUDGET WIRED' ELSE 'MISSING' END AS s1,
       CASE WHEN INSTR(h.source, 'budgetThresholds') > 0 THEN 'THRESHOLDS WIRED' ELSE 'MISSING' END AS s2,
       CASE WHEN INSTR(h.source, 'planCovGap') > 0 THEN 'PLAN KEPT' ELSE 'MISSING' END AS s3,
       CASE WHEN INSTR(h.source, 'l_negf') > 0 THEN 'NEGFUND KEPT' ELSE 'MISSING' END AS s4,
       CASE WHEN INSTR(h.source, 'l_cmtd') > 0 THEN 'COMMENTS KEPT' ELSE 'MISSING' END AS s5,
       CASE WHEN INSTR(h.source, 'taskName') > 0 THEN 'TASKNAME WIRED' ELSE 'MISSING' END AS s6
  FROM user_ords_handlers h
  JOIN user_ords_templates t ON t.id = h.template_id
  JOIN user_ords_modules m ON m.id = t.module_id
 WHERE m.name = 'gl.rest' AND t.uri_template = 'butil' AND h.method = 'GET';

SELECT setting_key, setting_value FROM prod.dct_module_settings
 WHERE setting_key LIKE 'BUD_UTIL%' ORDER BY setting_key;
