...
CREATE OR REPLACE FORCE EDITIONABLE VIEW "PROD"."AP_INVOICES_HEADER_V" ("INVOICE_ID", "INVOICE_NUMBER", "INVOICE_TYPE", "INVOICE_DATE", "INVOICE_DESCRIPTION", "INVOICE_STATUS", "SUPPLIER_NUMBER", "SUPPLIER_NAME", "BENEFICIARY_NAME", "BUSINESS_UNIT", "SUPPLIER_SITE", "PARTY_SITE_NAME", "PARTY_SITE_CITY", "PARTY_SITE_COUNTRY", "SUPPLIER_EMAIL", "INVOICE_CURRENCY", "INVOICE_AMOUNT", "TOTAL_TAX_CHARGED", "INVOICE_AMOUNT_AED", "CONVERSION_RATE", "CONVERSION_RATE_TYPE", "CONVERSION_DATE", "AMOUNT_PAID", "BALANCE_DUE", "PAYMENT_STATUS", "VALIDATION_STATUS", "ACCOUNTING_STATUS", "FUNDS_STATUS", "APPROVAL_STATUS", "PAYMENT_TERMS", "TERMS_DATE", "DUE_DATE", "PAYMENT_METHOD", "PAY_GROUP", "PAYMENT_CURRENCY", "PAY_ALONE_FLAG", "HEADER_PO_NUMBER", "VOUCHER_NUM", "INVOICE_SOURCE", "REQUESTER", "INTERCOMPANY_INVOICE", "BUDGET_DATE", "GL_DATE", "ENTERED_DATE", "INVOICE_RECEIVED_DATE", "CREATED_BY", "CREATED_DATE", "LAST_UPDATED_BY", "LAST_UPDATED_DATE", "CANCELLED_DATE", "CANCELLED_BY", "LINE_COUNT", "ITEM_LINE_COUNT", "TAX_LINE_COUNT", "DISTRIBUTION_COUNT", "PROJECT_COUNT", "TASK_COUNT", "PO_COUNT", "PR_COUNT", "RECEIPT_COUNT", "PO_NUMBERS", "PR_NUMBERS", "DISTRIBUTED_AMOUNT_AED") DEFAULT COLLATION "USING_NLS_COMP"  AS
  WITH line_agg AS (
  SELECT invoice_id,
         COUNT(*)                                              AS line_count,
...
SELECT
  ...
  i.business_unit_name                                         AS business_unit,
  ...
  i.accounting_status,
  i.funds_status,
  -- approval: Fusion codes -> display labels (col added to the OTBI analysis 2026-07-13)
  CASE i.approval_status WHEN 'WFAPPROVED' THEN 'Workflow Approved'
       ELSE INITCAP(i.approval_status) END                     AS approval_status,
  -- payment terms ------------------------------------------------------------
  i.payment_terms,
  i.terms_date,
  -- aging due date (user-confirmed 2026-07-14): received date (fallback
  -- created -> invoice date) + payment-terms credit days ('Net 30' -> 30);
  -- EVERY aging/overdue/days-past-due figure derives from this column
  TRUNC(COALESCE(i.invoice_received_date, i.created_date, i.invoice_date))
    + prod.dct_ap_pkg.terms_days(i.payment_terms)              AS due_date,
  i.payment_method,
  i.pay_group,
  i.payment_currency,
  i.pay_alone_flag,
...
pr_hdr AS (
  SELECT pr_number,
         ...
         MAX(pr_header_id)  AS pr_header_id
