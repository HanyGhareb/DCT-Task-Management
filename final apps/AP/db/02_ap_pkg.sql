    ...
    BEGIN
        ...
        IF p_sector IS NOT NULL THEN
            SELECT DISTINCT d.invoice_id BULK COLLECT INTO l_tmp
             ...
             WHERE dct_ap_pkg.in_list(p_sector, NVL(d.sector_name, 'Unclassified')) = 1;
            l_ids := l_ids MULTISET INTERSECT DISTINCT l_tmp;
        END IF;
        IF p_cc IS NOT NULL THEN
            SELECT DISTINCT d.invoice_id BULK COLLECT INTO l_tmp
              FROM prod.ap_invoice_distributions_v d
             WHERE dct_ap_pkg.in_list(p_cc, d.cost_center_code) = 1;
            l_ids := l_ids MULTISET INTERSECT DISTINCT l_tmp;
        END IF;
        IF p_project IS NOT NULL THEN
            SELECT DISTINCT d.invoice_id BULK COLLECT INTO l_tmp
              FROM prod.ap_invoice_distributions_v d
             WHERE dct_ap_pkg.in_list(p_project, d.project_number) = 1;
            l_ids := l_ids MULTISET INTERSECT DISTINCT l_tmp;
        END IF;
        IF p_task IS NOT NULL THEN
            SELECT DISTINCT d.invoice_id BULK COLLECT INTO l_tmp
              FROM prod.ap_invoice_distributions_v d
             WHERE UPPER(d.task_number) = UPPER(TRIM(p_task));
            l_ids := l_ids MULTISET INTERSECT DISTINCT l_tmp;
        END IF;
        IF p_etype IS NOT NULL THEN
            SELECT DISTINCT d.invoice_id BULK COLLECT INTO l_tmp
              FROM prod.ap_invoice_distributions_v d
             WHERE dct_ap_pkg.in_list(p_etype, d.expenditure_type) = 1;
            l_ids := l_ids MULTISET INTERSECT DISTINCT l_tmp;
        END IF;
        IF p_account IS NOT NULL THEN
            SELECT DISTINCT d.invoice_id BULK COLLECT INTO l_tmp
              FROM prod.ap_invoice_distributions_v d
             WHERE dct_ap_pkg.in_list(p_account, d.account_code) = 1;
            l_ids := l_ids MULTISET INTERSECT DISTINCT l_tmp;
        END IF;
        IF p_approp IS NOT NULL THEN
            SELECT DISTINCT d.invoice_id BULK COLLECT INTO l_tmp
              FROM prod.ap_invoice_distributions_v d
             WHERE dct_ap_pkg.in_list(p_approp, d.appropriation_code) = 1;
            l_ids := l_ids MULTISET INTERSECT DISTINCT l_tmp;
        END IF;
        IF p_po IS NOT NULL THEN
            SELECT DISTINCT d.invoice_id BULK COLLECT INTO l_tmp
              FROM prod.ap_invoice_distributions_v d
             WHERE TO_CHAR(d.po_number) = TRIM(p_po);
            l_ids := l_ids MULTISET INTERSECT DISTINCT l_tmp;
        END IF;
        IF p_pr IS NOT NULL THEN
            SELECT DISTINCT d.invoice_id BULK COLLECT INTO l_tmp
              FROM prod.ap_invoice_distributions_v d
             WHERE TO_CHAR(d.pr_number) = TRIM(p_pr);
            l_ids := l_ids MULTISET INTERSECT DISTINCT l_tmp;
        END IF;
        IF p_req IS NOT NULL THEN
            SELECT DISTINCT d.invoice_id BULK COLLECT INTO l_tmp
              FROM prod.ap_invoice_distributions_v d
             WHERE dct_ap_pkg.in_list(p_req, d.pr_preparer) = 1;
            l_ids := l_ids MULTISET INTERSECT DISTINCT l_tmp;
        END IF;
