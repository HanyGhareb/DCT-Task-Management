...
- **2026-07-13 (round 4b + invoice window, v1.5.0)** — cancelled invoices EXCLUDED by default (checkbox default off; /filters counts+LOVs follow the toggle via `inclcxl` bind, frontend re-fetches preserving selections); received-date fallback switched to invoice CREATION date (`COALESCE(invoice_received_date, created_date, invoice_date)` in pkg + trend); invoice drill window redesigned — master header region + top-right Summary card (amounts + status badges) + Lines/Distributions tabs + ⤢ full-screen maximize (Esc restores). 02→03→04 redeployed; SQL smoke PASS. APP_VERSION 1.5.0.
- **2026-07-13 (invoice window round 2, v1.6.0)** — Summary card fills the region height (statuses anchored); Audit info collapsible (created/updated by+date, drill emits lastUpdatedBy/Date); Distributions tab full GL-combination column w/ GL-Actuals-style 10-segment hover popover (drill joins dct_gl_coa_snap, canonical order). Approval status NOT in the Fusion extract (OTBI analysis change needed). 03→04 redeployed. APP_VERSION 1.6.0.
- **2026-07-13 (approval status, v1.6.1)** — `APPROVAL_STATUS` added to ATD_AP_INVOICES (OTBI change by user); dct_views_rebuild → 05 (display-label mapping) → 03 → 04; Summary card Approval badge row live. APP_VERSION 1.6.1.
- **2026-07-13 (UI polish, v1.6.2)** — ✓/✕ glyphs on Summary statuses, prominent Audit-info arrow, and badge--success/--warn/--danger finally DEFINED (were colorless everywhere). Frontend-only.
- **2026-07-13 (sector reconciliation, v1.6.3)** — dist facets: Item-only → NON-TAX rule (Accrual dists of PO-matched item lines were being dropped; coverage 3,381→5,390 invoices); Sector = per-invoice classification facet w/ (Multiple sectors) + Unclassified buckets — facet counts sum exactly to the KPI. 02→03→04 redeployed; verified Culture 2,054 / Multi 88 / Unclassified 93.
- **2026-07-13 (approval facet, v1.6.4)** — Approval Status search facet (counted checkbox group) + hidden register/CSV column at header level; p_appr in the engine + appr= across all handlers. 02→03→04 redeployed; verified 4,480/23.
- **2026-07-13 (Fusion deep-links, v1.6.5)** — invoice # opens the invoice in Oracle Fusion in a new tab (register all levels + drill drawer + invoice window; URL base in config.js). Frontend-only.
- **2026-07-13 (PO/PR deep-links, v1.7.0)** — shared `shared/js/fusionLinks.js` (reusable across apps); PR/PO numbers deep-link to Fusion at line/dist register levels + invoice-window tabs; 05 views carry PO_HEADER_ID/PR_HEADER_ID; 05→03→04 redeployed; fleet APP_VERSION bump (shared change).
