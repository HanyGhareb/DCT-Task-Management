...
- **Check duplicate using AI… (v1.13.0, bank-account evidence v1.15.0)** — `runAiDup()` header button: one `POST /ap/benef/dupcheck` clusters all beneficiary names into likely-duplicate groups (spacing/case/transliteration/name-order variants; each name's vendor bank account(s) ride the prompt as decisive same-identity evidence); results in a right-edge drawer (`aiOpen/aiLoading/aiError/aiMeta/aiGroups/aiShared`, Esc closes) with canonical name + confidence badge (`aiConfCls/aiConfTxt`) + reason + member stats table (incl. bank accounts), a **Shared bank accounts red-flag section** (`aiShared` — same account paid under 2+ different vendor names, platform-wide across all suppliers; vendor table w/ supplier no + amounts; `aiSharedTrunc` top-100 note), `aiExportCsv()` (BOM CSV, both sections) and verify-before-acting notes. AI config = the FL module's provider/model (server side).
