/**
 * authService.js — Session reader for Fusion BPM — Workflow Management (App 214).
...
function (config, api) {
  ...
  return {
    ...
    // Workflow administrator — mirrors the /wf/ designer + role-assignment
    // route gate (WF_ADMIN-or-SYS_ADMIN); the server check stays authoritative.
    isWfAdmin: function () {
      var user = this.getCurrentUser();
      return !!(user && user.roles && (user.roles.includes('WF_ADMIN') || user.roles.includes('SYS_ADMIN')));
    },
