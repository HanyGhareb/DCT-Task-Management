...
Admin/UD/
  ...
  UD-ADMIN-01_<First-Story-Title>/
    UD-ADMIN-01_<First-Story-Title>.md     ← the story pack (spec + Dev/Test/Deploy trackers)
    assets/                                 ← optional: mockups, diagrams, screenshots
  UD-ADMIN-02_<Next-Story>/
    ...
```

## Naming

- **ID:** `UD-ADMIN-<NN>` — app code + zero-padded 2-digit sequence (`UD-ADMIN-01`, `UD-ADMIN-02`, …).
  Sequence is **global-sequential per app** and never reused.
- **Folder:** `UD-ADMIN-<NN>_<Short-Title-Kebab>`.
...
*No stories yet. The first will be `UD-ADMIN-01_<Short-Title>` — create its folder here when work starts.*
