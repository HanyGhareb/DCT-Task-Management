...
11. ~~**Module-access management UI**~~ — ✅ **DONE 2026-07-13 (db/v2/49, APP_VERSION 4.6.3).** `DCT_MODULE_ROLES` is live: Admin → Modules → 🔑 Access modal sets which roles see each app in the module switcher (`GET/PUT /modules/:id/roles`); the shared shell hides denied entries via `GET /my/modules`. Zero rows = everyone (fail-open), SYS_ADMIN always sees all. ATD + GL registry rows gap-filled. **Server-side enforcement added same day (db/v2/50):** `DCT_REST.validate_session` returns 403 on every protected API call to a restricted module the user has no role for (direct-URL/curl override blocked; `/dct/` exempt), and the shell redirects out of a denied app at boot. Remaining follow-up: the Admin dashboard "modules directory" cards are not filtered (cosmetic — the data behind them is now gated).
