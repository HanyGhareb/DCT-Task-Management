...
-- =============================================================================
-- 12_pc_inbox_view.sql
-- PC: serve the approvals inbox from the SHARED worklist view.
-- Schema : ADMIN (ORDS metadata). Own SQLcl session.
-- Run    : AFTER db/v2/64 (views) and db/v2/66 (ADMIN synonyms).
--
-- Retires PC's private copy of the inbox query. Eligibility now has ONE
-- definition platform-wide (PROD.DCT_WF_INBOX_V). PC's own response keys are
...
PROMPT === PC: approvals inbox -> DCT_WF_INBOX_V ===

BEGIN
    ORDS.DEFINE_HANDLER(
        p_module_name => 'pc.rest',
...
END;
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('pc.rest approvals/pending now reads the shared inbox view.');
END;
/

PROMPT === 12_pc_inbox_view.sql complete ===
