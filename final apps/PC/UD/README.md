# UD — User Stories (Petty Cash, App 201)
...
PC/UD/
  ...
  UD-PC-01_<First-Story-Title>/
    UD-PC-01_<First-Story-Title>.md     ← the story pack (spec + Dev/Test/Deploy trackers)
    assets/                                 ← optional: mockups, diagrams, screenshots
  UD-PC-02_<Next-Story>/
    ...
```

## Naming

- **ID:** `UD-PC-<NN>` — app code + zero-padded 2-digit sequence (`UD-PC-01`, `UD-PC-02`, …).
  Sequence is **global-sequential per app** and never reused.
- **Folder:** `UD-PC-<NN>_<Short-Title-Kebab>`.
...
*No stories yet. The first will be `UD-PC-01_<Short-Title>` — create its folder here when work starts.*
