define(['knockout', 'services/reimbService'],
function (ko, reimbService) {
  'use strict';

  function AllReimbursementsViewModel() {
    var self = this;
    ...
    self.filtered = ko.computed(function () {
      ...
      return self.records().filter(function (r) {
        var matchQ = !q || r.reimbNumber.toLowerCase().includes(q) || r.employeeName.toLowerCase().includes(q) || r.pcNumber.toLowerCase().includes(q);
    ...
    reimbService.getAllReimbursements().then(function (list) {
      self.records(list);
      self.loading(false);
    }).catch(function () { self.loading(false); });
  }

  return AllReimbursementsViewModel;
});
