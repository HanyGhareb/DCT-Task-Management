# UD — User Stories (Analytics Loader, App 208)
...
ATD/UD/
  ...
  UD-ATD-01_<First-Story-Title>/
    UD-ATD-01_<First-Story-Title>.md     ← the story pack (spec + Dev/Test/Deploy trackers)
    assets/                                 ← optional: mockups, diagrams, screenshots
  UD-ATD-02_<Next-Story>/
    ...
```

## Naming

- **ID:** `UD-ATD-<NN>` — app code + zero-padded 2-digit sequence (`UD-ATD-01`, `UD-ATD-02`, …).
  Sequence is **global-sequential per app** and never reused.
- **Folder:** `UD-ATD-<NN>_<Short-Title-Kebab>`.
...
*No stories yet. The first will be `UD-ATD-01_<Short-Title>` — create its folder here when work starts.*
