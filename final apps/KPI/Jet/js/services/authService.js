/**
 * authService.js — Session reader for the Finance KPIs module (App 213).
...
function (config, api) {
  ...
  return {
    ...
    isKpiAdmin: function () {
      var user = this.getCurrentUser();
      return !!(user && user.roles && (user.roles.includes('KPI_ADMIN') || user.roles.includes('SYS_ADMIN')));
    },

    isKpiApprover: function () {
      var user = this.getCurrentUser();
      return !!(user && user.roles && (user.roles.includes('KPI_FIN_DIRECTOR') || user.roles.includes('SYS_ADMIN')));
    },
