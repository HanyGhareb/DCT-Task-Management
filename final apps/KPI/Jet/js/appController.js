...
function (ko, config, authService, notifService, i18n, shell) {
  ...
  function AppController() {
    ...
    shell.initBrand('kpi');
    shell.initAnnouncements('kpi', config.authBase);
    ...
    self.currentModule = shell.byKey('kpi');
    ...
    var NAV_GROUPS = [
      ...
      { id: 'insights', labelKey: 'nav.insightsGroup', auth: 'all', collapsed: ko.observable(false), items: [
          { id: 'reports', labelKey: 'nav.reports', icon: '&#128196;' } ] },
      { id: 'admin', labelKey: 'nav.adminGroup', auth: 'admin', collapsed: ko.observable(false), items: [
          { id: 'kpiList', labelKey: 'nav.kpiRegistry', icon: '&#9881;' } ] },
    ...
    self.navGroups = ko.computed(function () {
      ...
      return NAV_GROUPS.filter(function (g) {
        ...
        if (g.auth === 'admin') return _isKpiAdmin(user);
        return false;
      });
    });
    ...
    self._loadRoute = function (path) {
      ...
      require(['text!views/' + path + '.html', 'viewModels/' + path],
        ...
        function (err) { console.error('[KPIRouter] Failed to load route:', path, err); }
      );
    };
