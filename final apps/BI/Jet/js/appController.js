define(['knockout', 'services/config', 'services/authService', 'services/rptService', 'shared/i18n', 'shared/shell'],
function (ko, config, authService, rptService, i18n, shell) {
  'use strict';
  ...
  function AppController() {
    ...
    shell.initBrand('bi');
    shell.initAnnouncements('bi', config.authBase || config.apiBase);
    shell.initRegionTheme(config.authBase || config.apiBase, function () {
      return Promise.resolve(null);   // BI has no module-settings row; registry default brand
    });
    self.t       = i18n.t;
    self.lang    = i18n.lang;
    ...
    self.currentModule = shell.byKey('bi');
    ...
    self._loadRoute = function (path) {
      ...
      require(
        ...
        function (err) { console.error('[BIRouter] Failed to load route:', path, err); }
      );
    };
