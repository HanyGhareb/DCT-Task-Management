...
function (ko, config, authService, i18n, shell) {
  ...
  function AppController() {
    ...
    shell.initBrand('pay');
    shell.initAnnouncements('pay', config.authBase || config.apiBase);
    ...
    self.currentModule = shell.byKey('pay');
    ...
    self._loadRoute = function (path) {
      ...
      require(
        ...
        function (err) { console.error('[PAYRouter] Failed to load route:', path, err); }
      );
    };
