...
define(['services/api', 'services/config'], function (api, config) {
  ...
  return {
    ...
    // ── Employee change control (Phase 3.1) ──────────────────────────
    chgRegister:  function (payrollId, periodId) { return api.get('/changes/register' + qs({ payrollid: payrollId, periodid: periodId })); },
    chgCapture:   function (payrollId, periodId) { return api.post('/changes/capture', { payrollId: payrollId, periodId: periodId }); },
    chgItems:     function (regId, params)       { return api.get('/changes/' + regId + '/items' + qs(params)); },
    chgAll:       function (regId, params)       { return api.get('/changes/' + regId + '/all' + qs(params)); },
    chgConfirm:   function (regId, side, action, items) { return api.post('/changes/' + regId + '/confirm', { side: side, action: action, items: items || 'ALL' }); },
    ...
    companyDashDrill: function (id, metric, key) {
      return api.get('/companies/' + id + '/dashboard/drill' + qs({ metric: metric, key: key }));
    },
