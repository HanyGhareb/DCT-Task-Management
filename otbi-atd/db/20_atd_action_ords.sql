...
BEGIN
  ...
  FOR r IN (
    SELECT a.action_id, a.action_type, a.env_name, a.source_module, a.source_type,
           ...
           TO_CHAR(a.claimed_at,'YYYY-MM-DD HH24:MI') AS claimed_s,
           TO_CHAR(a.created_at,'YYYY-MM-DD HH24:MI') AS created_s,
           TO_CHAR(a.updated_at,'YYYY-MM-DD HH24:MI') AS updated_s
...
BEGIN
  ...
  FOR r IN (SELECT * FROM atd_action_request WHERE action_id = l_id) LOOP
    ...
    FOR h IN (SELECT new_status, old_status, comments,
                     TO_CHAR(changed_at,'YYYY-MM-DD HH24:MI') AS changed_s
