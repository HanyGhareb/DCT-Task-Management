...
db/v2/               ← All DCT_* DDL, packages, ORDS setup (source of truth)
  ...
  48_gl_cc_code_canonical.sql ← canonical-order rebuild of DCT_GL_CODE_COMBINATIONS.cc_code
                       (10-seg entity.cc.account.appr.bg.es.f1.f2.ic.program + new
...
| `DCT_GL_CODE_COMBINATIONS` | segment columns end in `_code` (`account_code`, not `account`); `cc_code` is a virtual column with the full 10-segment concatenation in the CANONICAL order (entity.cc.account.appr.bg.es.f1.f2.ic.program — db/v2/48, 2026-07-13) — use it for display. **Platform rule: EVERY exposed combination string uses this canonical order — wrap feed strings with `prod.dct_cc_canon`** |
